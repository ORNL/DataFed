...
class TestDataFedPythonAPICollectionCRUD(unittest.TestCase):
    def setUp(self):
        ...
        while status < 3:
            if count > 2:
                print(task_result)
                self.fail("Something went wrong task was unable to complete, attempt to create an allocation after 3 seconds failed, make sure all services are running.")
                break
            time.sleep(1)
            task_result = self._df_api.taskView(task_id)
            status = task_result[0].task[0].status 
    ...
    def test_collection_create_delete(self):
        ...
        title = "Materials"
        alias = "materials"
        col_result = self._df_api.collectionCreate(title=title, alias=alias, parent_id='root') 
        ...
        self.assertEqual(result_parent[0].path[0].item[0].id, f"c/u_{self._username}_root")
        ...
        while status < 3:
            if count > 2:
                print(task_result)
                self.fail("Something went wrong task was unable to complete, attempt to delete a colleciton after 3 seconds failed, make sure all services are running.")
                break
            time.sleep(1)
            task_result = self._df_api.taskView(task_id)
            status = task_result[0].task[0].status 
