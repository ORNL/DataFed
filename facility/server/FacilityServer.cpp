...
Server::Server(uint32_t a_port, const string &a_cert_dir, uint32_t a_timeout,
    ...
    : m_port(a_port), m_timeout(a_timeout), m_io_thread(0), m_maint_thread(0),
      m_num_threads(a_num_threads), m_io_running(false),
      ...
      m_xfr_thread(0), m_db_url(a_db_url), m_db_user(a_db_user),
      m_db_pass(a_db_pass), m_db_client(m_db_url, m_db_user, m_db_pass) {
            ...
            if ((*ixfr)->status != status) {
              ...
              if ((*ixfr)->mode == XM_PUT) {
                ...
                if ((*ixfr)->status == XS_SUCCEEDED) {
                  ...
                  if (!m_store.dataGetSize(string("/data/") +
                                               (*ixfr)->data_id.substr(2),
                                           file_size)) {
                    // TODO This should not happen. If it does something is very
                    // very wrong
                    DL_ERROR("Transfer succeeded but destination file does not "
                             "exist! Transfer ID: "
                             << (*ixfr)->id);
                    file_size = 0;
                  }
