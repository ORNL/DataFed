...
#define SET_MSG_HANDLER(proto_id, msg, func)                                   \
  m_msg_handlers[MsgBuf::findMessageType(proto_id, #msg)] = func
#define SET_MSG_HANDLER_DB(proto_id, rq, rp, func)                             \
  m_msg_handlers[MsgBuf::findMessageType(proto_id, #rq)] =                     \
      &Session::dbPassThrough<rq, rp, &CentralDatabaseClient::func>

map<uint16_t, Session::msg_fun_t> Session::m_msg_handlers;

Session::Session(asio::io_service &a_io_service, asio::ssl::context &a_context,
                 ISessionMgr &a_sess_mgr, const std::string &a_db_url,
                 const std::string &a_db_user, const std::string &a_db_pass)
    : m_sess_mgr(a_sess_mgr), m_socket(a_io_service, a_context), m_anon(true),
      m_in_buf(4096), m_db_client(a_db_url, a_db_user, a_db_pass) {
  ...
  m_socket.set_verify_callback(
      bind(&Session::verifyCert, this, placeholders::_1, placeholders::_2));

  cout << "ctor(" << this << "), m_anon: " << m_anon << "\n";
}

Session::~Session() { cout << "Session " << this << " deleted\n"; }

void Session::setupMsgHandlers() {
  ...
  if (init) {
    ...
    SET_MSG_HANDLER_DB(proto_id, RecordListRequest, RecordDataReply,
                       recordList);
    SET_MSG_HANDLER_DB(proto_id, RecordViewRequest, RecordDataReply,
                       recordView);
    SET_MSG_HANDLER_DB(proto_id, RecordFindRequest, RecordDataReply,
                       recordFind);
    SET_MSG_HANDLER_DB(proto_id, RecordCreateRequest, RecordDataReply,
                       recordCreate);
    SET_MSG_HANDLER_DB(proto_id, RecordUpdateRequest, RecordDataReply,
                       recordUpdate);
...
void Session::start() {
  ...
  m_socket.async_handshake(
      asio::ssl::stream_base::server, [this, self](error_code ec) {
        if (ec)
...
void Session::close() { m_socket.lowest_layer().close(); }
...
double Session::lastAccessTime() {
  return m_last_access.tv_sec + (m_last_access.tv_nsec * 1e-9);
}

void Session::handleCommError(const std::string &a_msg, error_code a_ec) {
  if (a_ec)
    DL_ERROR(a_msg << a_ec.category().name() << "[" << a_ec.value() << "] "
                   << a_ec.message());
...
bool Session::verifyCert(bool a_preverified,
                         asio::ssl::verify_context &a_context) {
...
void Session::readMsgHeader() {
  ...
  asio::async_read(
      m_socket,
      asio::buffer((char *)&m_in_buf.getFrame(), sizeof(MsgBuf::Frame)),
...
void Session::readMsgBody() {
  ...
  if (m_in_buf.getFrame().size) {
    ...
    asio::async_read(
        m_socket, asio::buffer(m_in_buf.getBuffer(), m_in_buf.getFrame().size),
        [this, self](error_code ec, size_t) {
          if (ec)
...
void Session::writeMsgHeader() {
  ...
  asio::async_write(
      m_socket,
      asio::buffer((char *)&m_out_buf.getFrame(), sizeof(MsgBuf::Frame)),
      [this, self](error_code ec, size_t) {
        if (ec)
          handleCommError("readMsgBody: ", ec);
...
void Session::writeMsgBody() {
  ...
  if (m_out_buf.getFrame().size) {
    ...
    asio::async_write(
        m_socket,
        asio::buffer(m_out_buf.getBuffer(), m_out_buf.getFrame().size),
        [this, self](error_code ec, size_t) {
          if (ec)
...
#define PROC_MSG_END                                                           \
  m_out_buf.serialize(reply);                                                  \
    ...
    DL_WARN("Session " << this                                                 \
                       << " unkown exception while processing message!");      \
    NackReply nack;                                                            \
    nack.set_err_code(ID_INTERNAL_ERROR);                                      \
  ...
  DL_TRACE("Sent: " << reply.DebugString());                                   \
    ...
    DL_ERROR("Session " << this << ": dynamic cast of msg buffer "             \
                        << &m_in_buf << " failed!");                           \
  }                                                                            \
  delete base_msg;                                                             \
  }                                                                            \
    ...
    DL_ERROR(                                                                  \
        "Session " << this                                                     \
                   << ": buffer parse failed due to unregistered msg type.");  \
  }

void Session::procMsgServerInfo() {
...
void Session::procMsgAuthenticate() {
...
void Session::procMsgSetLocalIdentity() {
...
/** @brief Creates identity for local environment and generate matching
 * credentials
 *
 ...
 * subsequent use.
 */
void Session::procMsgGenerateCredentials() {
  ...
  try {
    // TODO This process is BROKEN. There is nothing that prevents anyone from
    // replicating this process anywhere/
    // TODO We MUST SIGN the certificate with the HOST key to ensure they can
    // not be spoofed
    ...
    cmd = "openssl req -new -key " + key_file +
          " -subj /C=" + m_sess_mgr.getCountry() + "/O=" + m_sess_mgr.getOrg() +
          "/OU=" + m_sess_mgr.getUnit() + "/CN=" + m_uid + " -out " + csr_file;
    if (system(cmd.c_str()))
      EXCEPT(ID_SERVICE_ERROR, "CSR generation failed.");

    cmd = "openssl x509 -req -in " + csr_file + " -CA " +
          m_sess_mgr.getCertFile() + " -CAkey " + m_sess_mgr.getKeyFile() +
          " -CAcreateserial -out " + cert_file + " -days 1024 -sha256";
    if (system(cmd.c_str()))
...
void Session::procMsgGenerateKeys() {
...
void Session::procMsgGetPublicKey() {
...
void Session::procMsgDataDelete() {
...
void Session::procMsgRecordDelete() {
  ...
  // Delete record FIRST - If successful, this verifies that client has
  // permission and ID is valid
...
template <typename RQ, typename RP,
          void (CentralDatabaseClient::*func)(const RQ &, RP &)>
void Session::dbPassThrough() {
