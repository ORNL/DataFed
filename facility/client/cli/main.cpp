...
const char* StatusText[] = {"INITIAL", "ACTIVE", "INACTIVE", "SUCCEEDED",
                            "FAILED"};

typedef int (*cmd_func_t)();

struct CmdInfo {
  CmdInfo(const char* a_cmd_short, const char* a_cmd_long,
          const char* a_desc_short, const char* a_desc_long, cmd_func_t a_func)
      : cmd_short(a_cmd_short),
        cmd_long(a_cmd_long),
        desc_short(a_desc_short),
        desc_long(a_desc_long),
        func(a_func) {}

  void help(bool a_full = false) {
    cout << cmd_long;
    if (cmd_short.size()) cout << " (" << cmd_short << ")";
    cout << " - " << desc_short << "\n";
...
void addCommand(const char* a_cmd_short, const char* a_cmd_long,
                const char* a_desc_short, const char* a_desc_long,
                cmd_func_t a_func) {
  g_cmd_list.push_back(
      CmdInfo(a_cmd_short, a_cmd_long, a_desc_short, a_desc_long, a_func));
}

void buildCmdMap() {
  for (cmd_list_t::iterator c = g_cmd_list.begin(); c != g_cmd_list.end();
       c++) {
    if (c->cmd_short.size()) g_cmd_map[c->cmd_short] = &(*c);
    if (c->cmd_long.size()) g_cmd_map[c->cmd_long] = &(*c);
  }
}
...
string resolveIndexValue(const string& a_id) {
  if (a_id.size() == 1 || a_id.size() > 3 || a_id.back() != '.') return a_id;

  unsigned int i;

  for (i = 0; i < a_id.size() - 1; i++) {
    if (!isdigit(a_id[i])) return a_id;
  }

  if (g_id_index.size() == 0) EXCEPT(1, "No listing data for index lookup.");

  i = strtoul(a_id.c_str(), NULL, 10);
  if (i == 0 || i > g_id_index.size()) EXCEPT(1, "Index value out of range.");
...
string resolveID(const string& a_id) {
  ...
  if (id.size() > 2 && id[1] == '/') return id;

  if (id.find_first_of(":") != string::npos) return id;
...
void printUsers(spUserDataReply a_reply) {
  ...
  if (g_out_form == CSV) cout << "\"UserID\",\"Name\",\"Email\",\"Admin\"\n";
  ...
  if (a_reply->user_size()) {
    for (int i = 0; i < a_reply->user_size(); i++) {
      ...
      if (g_out_form == TEXT) {
        ...
        cout << "Name     " << user.name() << "\n";
        if (user.has_email()) cout << "Email    " << user.email() << "\n";
  ...
  if (a_reply->proj_size()) {
    ...
    for (int i = 0; i < a_reply->proj_size(); i++) {
      ...
      if (g_out_form == TEXT) {
        ...
        if (proj.has_desc()) cout << "Desc    " << proj.desc() << "\n";
        if (proj.has_owner()) cout << "Owner   " << proj.owner() << "\n";
        if (proj.has_ct()) {
          t = (time_t)proj.ct();
          pTM = localtime(&t);
...
void printData(spRecordDataReply a_rep) {
  ...
  if (g_out_form == CSV)
    cout << "\"DataID\",\"Alias\",\"Title\",\"Desc\",\"Topic\",\"Keywords\","
            "\"Owner\",\"Locked\",\"Size\",\"Repo\",\"Parent\",\"Uploaded\","
  ...
  if (a_rep->data_size()) {
    ...
    for (int i = 0; i < a_rep->data_size(); i++) {
      ...
      if (g_out_form == TEXT) {
        cout << "DataID   " << rec.id();
        if (rec.has_alias()) cout << " (" << rec.alias() << ")";
        cout << "\nTitle    " << rec.title() << "\n";
        cout << "Desc     ";
        if (rec.has_desc()) {
          ...
          for (int j = 0; j < rec.deps_size(); j++) {
            ...
            switch (dep.type()) {
              ...
              case DEP_IS_NEW_VERSION_OF:
                cout << (dep.dir() == DEP_OUT ? "New version of "
                                              : "Deprecated by ");
                break;
              default:
                break;
            ...
            cout << dep.id();
            if (dep.has_alias()) cout << " (" << dep.alias() << ")";
            cout << "\n";
          }
        }
      } else {
...
void printCollData(spCollDataReply a_reply) {
  ...
  if (g_out_form == CSV)
    cout << "\"CollID\",\"Alias\",\"Title\",\"Desc\",\"Owner\",\"Parent\","
            "\"Created\",\"Updated\"\n";
  ...
  if (a_reply->coll_size()) {
    ...
    for (int i = 0; i < a_reply->coll_size(); i++) {
      ...
      if (g_out_form == TEXT) {
        cout << "CollID  " << coll.id() << "\n";
        if (coll.has_alias()) cout << "Alias   " << coll.alias() << "\n";
        cout << "Title   " << coll.title() << "\n";
        if (coll.has_desc()) cout << "Desc    " << coll.desc() << "\n";
        if (coll.has_owner()) cout << "Owner   " << coll.owner() << "\n";
...
void printListing(spListingReply a_reply) {
  ...
  if (g_out_form == CSV) cout << "\"id\",\"alias\",\"title\",\"locked\"\n";
...
void printXfrData(spXfrDataReply a_reply) {
  ...
  if (a_reply->xfr_size()) {
    ...
    if (g_out_form == CSV)
      cout << "\"TransID\",\"DataID\",\"Mode\",\"Status\",\"Error\",\"Path\","
              "\"StatusTS\"\n";
    ...
    for (int i = 0; i < a_reply->xfr_size(); i++) {
      ...
      if (g_out_form == TEXT) {
        ...
        if (xfr.has_err_msg()) cout << "Error     " << xfr.err_msg() << "\n";
    ...
    if (g_out_form == TEXT) cout << "No matching transfers\n";
  }
}
...
int help() {
  if (g_args.size() == 0) {
    ...
    for (cmd_list_t::iterator icmd = g_cmd_list.begin();
         icmd != g_cmd_list.end(); ++icmd) {
      cout << "  ";
      icmd->help();
    }
  } else {
...
spRecordDataReply createRecord() {
  ...
  if (g_meta_file.size()) {
    ...
    string metadata((istreambuf_iterator<char>(inf)),
                    istreambuf_iterator<char>());

    inf.close();

    return g_client->recordCreate(
        g_title, g_desc.size() ? g_desc.c_str() : 0,
        g_alias.size() ? g_alias.c_str() : 0,
        g_keyw.size() ? g_keyw.c_str() : 0,
        g_topic.size() ? g_topic.c_str() : 0, metadata.c_str(), par.c_str(),
        g_repo.size() ? g_repo.c_str() : 0, &g_dep_add);
  } else {
    return g_client->recordCreate(
        g_title, g_desc.size() ? g_desc.c_str() : 0,
        g_alias.size() > 2 ? g_alias.c_str() : 0,
        g_keyw.size() ? g_keyw.c_str() : 0,
        g_topic.size() ? g_topic.c_str() : 0,
        g_meta.size() ? g_meta.c_str() : 0, par.c_str(),
        g_repo.size() ? g_repo.c_str() : 0, &g_dep_add);
  }
}
...
spRecordDataReply updateRecord(const string& a_id) {
  if (g_meta_file.size()) {
    ...
    return g_client->recordUpdate(a_id, g_title.size() ? g_title.c_str() : 0,
                                  g_desc.size() ? g_desc.c_str() : 0,
                                  g_alias.size() ? g_alias.c_str() : 0,
                                  g_keyw.size() ? g_keyw.c_str() : 0,
                                  g_topic.size() ? g_topic.c_str() : 0,
                                  metadata.c_str(), !g_meta_replace, &g_dep_add,
                                  &g_dep_rem, g_dep_clear);
  } else {
    return g_client->recordUpdate(a_id, g_title.size() ? g_title.c_str() : 0,
                                  g_desc.size() ? g_desc.c_str() : 0,
                                  g_alias.size() > 2 ? g_alias.c_str() : 0,
                                  g_keyw.size() ? g_keyw.c_str() : 0,
                                  g_topic.size() ? g_topic.c_str() : 0,
                                  g_meta.size() ? g_meta.c_str() : 0,
                                  !g_meta_replace, &g_dep_add, &g_dep_rem,
                                  g_dep_clear);
  }
}

int query_text() {
  if (g_args.size() == 0) return -1;

  size_t p = g_cmd_str.find_first_of(' ');
  if (p == string::npos) return -1;

  string query = "{\"quick\":\"" + escapeJSON(g_cmd_str.substr(p + 1)) +
                 "\",\"scopes\":[{\"scope\":1},{\"scope\":3},{\"scope\":4},{"
                 "\"scope\":5}]}";
...
int query_meta() {
  ...
  string query = "{\"meta\":\"" + escapeJSON(g_cmd_str.substr(p)) +
                 "\",\"scopes\":[{\"scope\":1},{\"scope\":3},{\"scope\":4},{"
                 "\"scope\":5}]}";
...
int data_get() {
  if (g_args.size() == 1) {
    ...
    switch (g_out_form) {
      case TEXT:
        // cout << "TransID  " << xfr.id() << "\nStatus   " <<
        // StatusText[xfr.status()] << "\nPath     " << xfr.local_path() <<
        // "\n";
        break;
      case CSV:
        cout << "\"TransID\",\"Status\",\"Path\"\n";
        // cout << "\"" << xfr.id() << "\",\"" << StatusText[xfr.status()] <<
        // "\",\"" << xfr.local_path() << "\"\n";
        break;
      case JSON:
        // cout << "{\"id\":\"" << xfr.id() << "\",\"status\":\"" <<
        // StatusText[xfr.status()] << "\",\"local_path\":\"" <<
        // xfr.local_path()
        // << "\"}\n";
        break;
    }
...
int data_put() {
  ...
  switch (g_out_form) {
    ...
    case CSV:
      ...
      // cout << "\"" + data_id + "\",\"" << xfr.id() << "\",\"" <<
      // StatusText[xfr.status()] << "\",\"" << xfr.local_path() << "\"\n";
      break;
    case JSON:
      // cout << "{\"data_id\":\"" + data_id + "\",\"id\":\"" << xfr.id() <<
      // "\",\"status\":\"" << StatusText[xfr.status()] <<
      // "\",\"local_path\":\""
      // << xfr.local_path() << "\"}\n";
      break;
...
int coll_create() {
  ...
  if (!g_title.size()) EXCEPT_PARAM(1, "Title is required");
  ...
  spCollDataReply rep = g_client->collCreate(
      g_title, g_desc.size() ? g_desc.c_str() : 0,
      g_alias.size() > 2 ? g_alias.c_str() : 0, par.c_str());
  printCollData(rep);
...
int coll_update() {
  ...
  spCollDataReply rep = g_client->collUpdate(
      resolveID(g_args[0]), g_title.size() ? g_title.c_str() : 0,
      g_desc.size() ? g_desc.c_str() : 0,
      g_alias.size() > 2 ? g_alias.c_str() : 0);
  printCollData(rep);
...
int xfr_list() {
  if (g_args.size() == 0) {
    ...
    spXfrDataReply xfr =
        g_client->xfrList(since ? &since : 0, g_from ? &g_from : 0,
                          g_to ? &g_to : 0, g_status >= 0 ? &status : 0);
    printXfrData(xfr);
  } else if (g_args.size() == 1) {
...
int select() {
  if (g_args.size() == 1) {
    ...
    if (new_sel.compare(0, 2, "p/") == 0) {
      ...
      if (new_sel.compare(0, 2, "u/") != 0) new_sel = "u/" + new_sel;
...
int wc() {
  if (g_args.size() == 0) {
    ...
    if (!fin) {
      spCollDataReply rep = g_client->collView(id);
      if (!rep->coll_size()) EXCEPT(1, "Invalid collection");
      ...
      if (rep->coll(0).owner() != g_cur_sel) {
        ...
        g_cur_alias_prefix = "u:" + g_cur_sel.substr(2) + ":";
        cout << "Switched to " << (g_cur_sel[0] == 'u' ? "user" : "project")
             << ": " << g_cur_sel << "\n";
      }
    } else {
      g_cur_col = id;
...
enum OptionResult { OPTS_OK, OPTS_HELP, OPTS_VERSION, OPTS_ERROR };

OptionResult processArgs(int a_argc, const char** a_argv,
                         po::options_description& a_opts_desc,
                         po::positional_options_description& a_opts_pos,
                         bool a_throw = true) {
  g_wait = false;
  g_title.clear();
  g_desc.clear();
  ...
  try {
    ...
    po::store(po::command_line_parser(a_argc, a_argv)
                  .options(a_opts_desc)
                  .positional(a_opts_pos)
                  .run(),
              opt_map);
    po::notify(opt_map);

    if (opt_map.count("help")) return OPTS_HELP;

    if (opt_map.count("version")) return OPTS_VERSION;

    if (g_cfg_file.size()) {
      ifstream optfile(g_cfg_file.c_str());
      if (!optfile.is_open())
        EXCEPT_PARAM(ID_CLIENT_ERROR,
                     "Could not open config file: " << g_cfg_file);
...
int exit_shell() { exit(0); }

int main(int a_argc, char** a_argv) {
  addCommand("?", "help", "Show help",
             "Use 'help <cmd>' to show help for a specific command.", help);

  // Data Commands
  addCommand("dv", "data-view", "View data record",
             "<id>\n\nViews fields of specified data record. The <id> argument "
             "may be an identifier or an alias.",
             data_view);
  addCommand(
      "dc", "data-create", "Create data record",
      "-t <title> [-a] [-d] [-md|-f [-r]]\n\nCreates a new data record using "
      "fields provided via options (see general help for option descriptions).",
      data_create);
  addCommand(
      "du", "data-update", "Update data record",
      "<id> [-t] [-a] [-d] [-md|-f [-r]]\n\nUpdates an existing data record "
      "using fields provided via options (see general help for option "
      "descriptions).  The <id> argument may be an identifier or an alias.",
      data_update);
  addCommand("get", "data-get", "Get data from repository",
             "<id> <dest>\n\nTransfer raw data from repository and place in a "
             "specified destination directory. The <id> parameter may be "
             "either a data identifier or an alias. The <dest> parameter is "
             "the destination path including a globus end-point prefix (if no "
             "prefix is specified, the default local end-point will be used). "
             "If no destination is specified, this command will return a local "
             "path to the (read-only) raw data file if one exists.",
             data_get);
  addCommand(
      "put", "data-put", "Put data into repository",
      "[id] <src> [-t title] [-d desc] [-a alias] [-m metadata |-f "
      "meta-file]\n\nTransfer raw data from the specified <src> path to the "
      "repository. If the 'id' parameter is provided, the record with the "
      "associated identifier (or alias) will receive the data; otherwise a new "
      "data record will be created (see help on data command for details). The "
      "source path may include a globus end-point prefix; however, if none is "
      "specified, the default local end-point will be used.",
      data_put);
  addCommand("", "data-clear", "Clear raw data",
             "<id>\n\nDeletes raw data associated with an existing data "
             "record. The <id> argument may be an identifier or an alias.",
             data_clear);
  addCommand("", "data-delete", "Delete data record",
             "<id>\n\nDeletes an existing data record, including raw data. The "
             "<id> argument may be an identifier or an alias.",
             data_delete);

  // Collection commands
  addCommand(
      "cv", "coll-view", "View collection record",
      "<id>\n\nView fields of specified collection record. The <id> argument "
      "may be an identifier or an alias. This command does not list items "
      "linked to the collection; for this, see the \"ls\" command.",
      coll_view);
  addCommand("cc", "coll-create", "Create collection record",
             "-t <title> [-a] [-d]\n\nCreates a new collection record using "
             "fields provided via options (see general help for option "
             "descriptions). The new collection is created as a child of the "
             "current working collection (root by default).",
             coll_create);
  addCommand(
      "cu", "coll-update", "Update collection record",
      "<id> [-t] [-a] [-d]\n\nUpdates an existing collection record using "
      "fields provided via options (see general help for option descriptions). "
      "The <id> argument may be an identifier or an alias.",
      coll_update);
  addCommand("", "coll-delete", "Delete collection record",
             "<id>\n\nDelete collection and all contained data.", coll_delete);

  // Query
  addCommand(
      "qt", "query-text", "Find data by text query",
      "<query>\n\nReturns a list of all data records that match specified "
      "query (see documentation for query language description).",
      query_text);
  addCommand(
      "qm", "query-meta", "Find data by metadata query",
      "<query>\n\nReturns a list of all data records that match specified "
      "query (see documentation for query language description).",
      query_meta);
  addCommand("ql", "query-list", "List saved queries",
             "\n\nReturns a list of saved queries.", query_list);
  addCommand("qv", "query-view", "View saved query",
             "<id>\n\nView details of saved query identified by <id>.",
             query_view);
  addCommand("qx", "query-exec", "Execute a saved query",
             "<id>\n\nExecute a saved query identified by <id>.", query_exec);

  // Transfer related commands
  addCommand("xl", "xfr-list", "List data transfers",
             "[<id>] [--since] [--from] [--to] [--status]\n\nList details of "
             "specified transfer (using <id>) or data transfers that match "
             "specified options.",
             xfr_list);
  addCommand("xs", "xfr-status", "Get data transfer status",
             "<id>\n\nGet status of data transfer specified by <id> parameter.",
             xfr_status);
  addCommand("epg", "ep-get", "Get default end-point",
             "\n\nGet current default end-point.", ep_get);
  addCommand("eps", "ep-set", "Set default end-point",
             "<name/uuid>\n\nSet current default end-point to specified legacy "
             "name or UUID.",
             ep_set);
  addCommand("epl", "ep-list", "List recent end-points",
             "\n\nList recently used end-points.", ep_list);

  addCommand("u", "user", "List/view users by affiliation",
             "<cmd>\n\nList or view user depending on value of <cmd> argument: "
             "\"collab\" lists collaborators (team members, users granted "
             "explicit access, etc.), \"shared\" lists users sharing data with "
             "current user, or specify a user ID to view a specific user.",
             user);

  addCommand(
      "p", "project", "List/view projects by affiliation",
      "project <cmd/id>\n\nList (m)y projects, (t)eam projects, or projects "
      "with (s)hared access, or view project information if an ID is given.",
      project);

  // File-system-like commands
  addCommand(
      "", "sel", "Select user or project",
      "[<id>]\n\nSelect the specified user or project for collection "
      "navigation. If no id is provided, prints the current user or project.",
      select);
  // addCommand( "", "pwc", "Print working collection","\n\nPrint current
  // workingcollection with parent hierarchy.", pwc );
  addCommand("", "wc", "Print/change working collection",
             "<id/cmd>\n\nPrints working collection if no arguments specified; "
             "otherwise changset working collection to specifed value. The "
             "'id/cmd' argument can be a collection ID or alias, '/' for the "
             "root collection, or '..' to move up one collection.",
             wc);
  addCommand(
      "", "ls", "List current collection",
      "[<id/cmd>]>\n\nList contents of current working collection or specified "
      "location. The 'id/cmd' argument can be a collection ID or alias, '/' "
      "for the root collection, '..' for the parent of the current working "
      "collection, or \".\" (or omitted) for the current collection.",
      ls);
  addCommand("ln", "link", "Link item into a collection",
             "<id> [<coll_id>]\n\nLinks a data record or collection into the "
             "specified collection. The <coll_id> paramter may be a collection "
             "ID or alias, \"/\" for root (of the current user or project), "
             "\"..\" for the parent of the current collection, or \".\" (or "
             "omitted) for the current collection. Note that if the item being "
             "linked is a collection, it will be unlinked from it's current "
             "location before being linked into the new location.",
             link);
  addCommand("ul", "unlink", "Unlink item from a collection",
             "<id> [<coll_id>]>\n\nUnlinks a data record or collection from "
             "the specified collection. The <coll_id> paramter may be \"/\" "
             "for root, \"..\" for the parent of the current collection, or "
             "\".\" (or omitted) for the current collection. Note that if the "
             "item being unlinked has no other links, it will be unlinked from "
             "the specified location and re-linked into the root collection.",
             unlink);
  addCommand("mv", "move", "Move link item to new collection",
             "<id> [<src_id>] <dest_id>>\n\nMoves item links from source "
             "collection <src_id> to destination collection <dest_id>. If the "
             "<src_id> paramter is omitted, the current collection is used. "
             "Collection ids may be \"/\" for root, \"..\" for the parent of "
             "the current collection, or \".\" for the current collection.",
             move);
  addCommand("", "setup", "Setup local environment",
             "\n\nSetup the local environment for non-interactive use by "
             "installing encryption keys in facility-specified location.",
             setup);
  addCommand("", "status", "Check status of core server",
             "\n\nGets core server status to to verify communication and "
             "system readiness.",
             ping);
  ...
  if (tmp) service_cred_dir = tmp;

  tmp = getenv("SDMS_CLIENT_CRED_DIR");
  if (tmp) client_cred_dir = tmp;
  ...
  opts_startup.add_options()("help,?", "Show help")(
      "version", "Show version number")("client-cred-dir,C",
                                        po::value<string>(&client_cred_dir),
                                        "Client credentials directory")(
      "serv-cred-dir,S", po::value<string>(&service_cred_dir),
      "SDMS service credentials directory")("host,H", po::value<string>(&host),
                                            "Service hostname/IP")(
      "port,P", po::value<uint16_t>(&port), "Service port")(
      "cfg", po::value<string>(&g_cfg_file), "Use config file for options")(
      "login,L", po::bool_switch(&manual_auth)->default_value(false),
      "Manually login to SDMS")(
      "sel", po::value<string>(&g_select),
      "Select user or project prior to executing command");

  g_opts_command.add_options()("wait,w",
                               po::bool_switch(&g_wait)->default_value(false),
                               "Block until command completes")(
      "title,t", po::value<string>(&g_title),
      "Specify title for create/update commands")(
      "desc,d", po::value<string>(&g_desc),
      "Specify description for create/update commands")(
      "alias,a", po::value<string>(&g_alias),
      "Specify alias for create/update commands")(
      "keyw,k", po::value<string>(&g_keyw),
      "Specify keywords for data create/update commands")(
      "top", po::value<string>(&g_topic),
      "Specify topic for data create/update commands")(
      "par,p", po::value<string>(&g_parent),
      "Specify parent collection ID or alias when creating new data record or "
      "collection")(
      "md,m", po::value<string>(&g_meta),
      "Specify metadata (JSON format) for create/update commands")(
      "md-file,f", po::value<string>(&g_meta_file),
      "Specify filename to read metadata from (JSON format) for create/update "
      "commands")(
      "md-replace,r", po::bool_switch(&g_meta_replace),
      "Replace existing metadata instead of merging with existing fields")(
      "dep-add,A", po::value<vector<string>>(&g_dep_add),
      "Add dependency (id/alias,type) for data create/update (repeat for "
      "multiple)")(
      "dep-rem,R", po::value<vector<string>>(&g_dep_rem),
      "Remove dependency (id/alias) for data update (repeat for multiple)")(
      "dep-clear,C", po::bool_switch(&g_dep_clear),
      "Clears all dependencies for data update (use with -A to set "
      "dependencies)")(
      "repo", po::value<string>(&g_repo),
      "Use specific storage allocation by repo ID for new data")(
      "text,T", po::bool_switch(&g_out_text),
      "Output in TEXT format (default)")(
      "json,J", po::bool_switch(&g_out_json),
      "Output in JSON format (default is TEXT)")(
      ...
      ("verbose,v", po::bool_switch(&g_details),
       "Show extra details for supported commands")(
          ...
          "Specify time since 'now' in seconds")(
  ...
  opts_hidden.add_options()("cmd", po::value<string>(&g_cmd), "Command to run")(
  ...
  try {
    OptionResult res =
        processArgs(a_argc, (const char**)a_argv, opts_all, opts_pos);

    if (g_cmd.size()) non_interact = true;

    if (res == OPTS_HELP) {
      cout << "SDMS CLI Client, ver. " << VER_MAJOR << "." << VER_MINOR << "."
           << VER_BUILD << "\n";
      ...
      cout << "\n\nRunning the sdms CLI without specifying a command starts an "
              "interactive shell.\n\n";
      return 1;
    }
    ...
    if (!manual_auth && !Client::verifyCredentials(client_cred_dir)) {
      if (non_interact) EXCEPT(0, "Manual authentication required");

      cerr << "No client credentials found, manual authentication required.\n";
      manual_auth = true;
    }

    Client client(host, port, timeout, service_cred_dir, client_cred_dir,
                  !manual_auth);

    string uid = client.start();

    if (def_ep) client.setDefaultEndpoint(def_ep);

    if (manual_auth || !uid.size()) {
      if (!manual_auth)
        cerr << "Client credentials are invalid, manual authentication "
                "required.\n";
      ...
      g_cur_sel = uname;
      if (g_cur_sel.compare(0, 2, "u/") != 0) g_cur_sel = "u/" + g_cur_sel;
    } else {
      if (!non_interact) cout << "Authenticated as " << uid << "\n";
      g_cur_sel = uid;
    }
    ...
    if (g_cmd.size()) {
      ...
      if (icmd != g_cmd_map.end()) {
        ...
        if (ec < 0) EXCEPT_PARAM(0, "Invalid arguments to command " << g_cmd);
      ...
      while (1) {
        ...
        try {
          ...
          if (processArgs(tok.tokens().size(), &tok.tokens()[0], opts_console,
                          opts_pos, false) == OPTS_OK) {
            if (g_cmd.size()) {
              /*
              cout << "cmd:["<<g_cmd<<"],args:";
              for ( vector<string>::iterator a = g_args.begin(); a !=
              g_args.end(); ++a ) cout << "["<<*a<<"]"; cout << "\n";
              */
