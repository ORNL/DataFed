...
struct CmdInfo {
  CmdInfo(const char *a_cmd_short, const char *a_cmd_long,
      ...
      : cmd_short(a_cmd_short), cmd_long(a_cmd_long), desc_short(a_desc_short),
        desc_long(a_desc_long), func(a_func) {}
...
int data_get() {
  if (g_args.size() == 1) {
    ...
    case JSON:
      ...
      // StatusText[xfr.status()] << "\",\"local_path\":\"" << xfr.local_path()
      // << "\"}\n";
      break;
    }
...
int data_put() {
  ...
  case JSON:
    ...
    // "\",\"status\":\"" << StatusText[xfr.status()] << "\",\"local_path\":\""
    // << xfr.local_path() << "\"}\n";
    break;
  }
