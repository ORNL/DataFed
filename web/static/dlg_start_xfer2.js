function dlgStartTransfer2( a_mode, a_data ) {
    ...
    frame.html( "<div class='ui-widget'>\
        ...
        Endpoint:\
        <div style='width:100%'><select id='matches' disabled><option disabled selected>No Matches</option></select></div> \
        <div style='padding:.25em' align=right>\
        <button class='btn' id='activate' disabled>Activate</button>&nbsp<button class='btn' id='browse' disabled>Browse</button></div><br>\
        <span id='prefix'>Source</span>:<br>\
    ...
    matches.on('selectmenuchange', function( ev, ui ) {
        if ( ep_list && ui.item ){
            ...
            cur_ep_id = ep.id;
            cur_ep = (ep.canonical_name || ep.id);
            path_in.val( cur_ep + (ep.default_directory?ep.default_directory:"/"));
    ...
    $("#activate",frame).on('click',function(){
        ...
        window.open('https://www.globus.org/app/endpoints/'+ encodeURIComponent(cur_ep_id) +'/activate','');
    });
    ...
    function inTimerExpired(){
        ...
        if ( ep != cur_ep ){
            ...
            epView( cur_ep, function( ok, data ){
                if ( ok && !data.code ){
                    console.log( "OK", data );

                    var html = "<option title='" + data.description + "'>" + (data.display_name || data.canonical_name || data.id) + " (" + (data.activated?Math.floor( data.expires_in/3600 ) + " hrs":"inactive") + ")</option>";
                    ...
                    epAutocomplete( cur_ep, function( ok, data ){
