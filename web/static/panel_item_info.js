...
var tree_opts1 = {
    ...
    lazyLoad: function(ev, data) {
        data.result = {
            url: api.annotationView_url(data.node.data.parentId),
            cache: false
        };
    },
...
function showSelectedItemForm(item) {
    var i, tmp, date = new Date(),
        t = item.id.charAt(0),
        text, cls, title, type, icon;
    ...
    switch (t) {
        ...
        case 'r':
            type = "Allocation";
            title = "Allocation for " + (item.user.startsWith("u/") ? " user " : " project ") + item.user;
            cls = ".sia";
            break;
        case 'q':
            type = "Saved Query";
