        ...
        if (ok) {
            ...
            $("#dlg_topic_tree", frame).fancytree({
                ...
                lazyLoad: function(event, data) {
                    data.result = {
                        url: api.topicListTopics_url(data.node.key, data.node.data.offset, settings.opts.page_sz),
                        cache: false
                    };
                },
                postProcess: function(event, data) {
                    ...
                    if (data.response.offset > 0 || data.response.total > (data.response.offset + data.response.count)) {
                        ...
                        data.result.push({
                            title: "<button class='btn small''" + (page == 1 ? " disabled" : "") + " onclick='pageLoadTopic(\"" + data.node.key +
                                ...
                                "\"," + (pages - 1) * settings.opts.page_sz + ")'>Last</button>",
                            folder: false,
                            icon: false,
                            checkbox: false,
                            hasBtn: true
