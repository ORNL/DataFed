...
export function show( a_cur_owner, a_new_owner, a_reply, cb ){
    var content =
        "<p>This operation will initiate a background task to transfer ownership of " + a_reply.totCnt + " record(s) with " + util.sizeToString( a_reply.actSize ) +
    ...
    for ( var i in a_reply.alloc ){
        ...
        content += "<option value='"+alloc.repo+"'>" + alloc.repo.substr(5) + " -- " + util.sizeToString( alloc.dataSize ) + " used of " + util.sizeToString( alloc.dataLimit ) + " total, " + free + "% free</option>";
    }
