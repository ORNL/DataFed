...
export function show(item) {
    ...
    function buildPermList(a_div_id, a_inh, a_mode) {
        ...
        children = [{
            title: "Read Rec.",
            inh: a_inh,
            key: model.PERM_RD_REC
        }];
        if (a_mode & DATA_MODE) {
            children.push({
                title: "Read Meta",
                inh: a_inh,
                key: model.PERM_RD_META
            });
            children.push({
                title: "Read Data",
                inh: a_inh,
                key: model.PERM_RD_DATA
            });
        }
        if (a_mode & COLL_MODE) {
            children.push({
        ...
        children = [{
            title: "Write Rec.",
            inh: a_inh,
            key: model.PERM_WR_REC
        }];
        if (a_mode & DATA_MODE) {
            children.push({
                title: "Write Meta",
                inh: a_inh,
                key: model.PERM_WR_META
            });
            children.push({
                title: "Write Data",
                inh: a_inh,
                key: model.PERM_WR_DATA
            });
        }
        if (a_mode & COLL_MODE) {
            children.push({
        ...
        src.push({
            ...
            children: [{
                title: "Delete",
                inh: a_inh,
                key: model.PERM_DELETE
            }, {
                title: "Share",
                inh: a_inh,
                key: model.PERM_SHARE
            }, {
                title: "Lock",
                inh: a_inh,
                key: model.PERM_LOCK
                ...
                if (data.node.isSelected()) {
                    ...
                    switch (perm) {
                        case model.PERM_RD_REC:
                            other = model.PERM_BAS_WRITE | model.PERM_RD_META | model.PERM_RD_DATA | model.PERM_LIST | model.PERM_BAS_ADMIN;
                            break;
                        ...
                        case model.PERM_BAS_READ:
                            other = model.PERM_WR_REC | model.PERM_WR_META | model.PERM_WR_DATA | model.PERM_LINK | model.PERM_CREATE | model.PERM_BAS_ADMIN;
                            break;
                    }
                    if (data.node.data.inh) {
                        cur_rule.inhgrant &= ~perm;
    ...
    function setPermsFromRule(rule) {
        ...
        if (is_coll) {
            for (i = 1; i <= model.PERM_MAX; i *= 2) {
                ...
                if (n)
                    n.setSelected((rule.inhgrant & i) != 0, {
                        noEvents: true
                    });
            }
        }
