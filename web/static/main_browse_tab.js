...
var frame = $("#content"),
    ...
    keyNav, keyNavMS,
    pasteItems = [],
    pasteSourceParent, pasteCollections,
    SS_TREE = 0,
    SS_CAT = 1,
    SS_PROV = 2,
    ...
    search_panel, cat_panel, cat_init = true,
    graph_panel;
            ...
            if (ok) {
                if (reply.totCnt == 0) {
                    ...
                    dialogs.dlgConfirmChoice("Confirm Change Record Allocation", "This operation will transfer " + reply.actCnt + " record(s) (out of " + reply.totCnt +
                        ...
                        reply.recLimit + " available. Pending transfers may alter the amount of space available on target allocation.", ["Cancel", "Confirm"],
                        function(choice) {
                            if (choice == 1) {
...
function handleQueryResults(data) {
    ...
    if (data.item && data.item.length > 0) {
        ...
        if (data.offset > 0 || data.total > (data.offset + data.count)) {
            ...
            results.push({
                title: "<button class='btn btn-icon-tiny srch-pg-first''" + (page == 0 ? " disabled" : "") + "><span class='ui-icon ui-icon-triangle-1-w-stop'></span></button> \
...
export function searchPanel_GetSelection() {
    ...
    for (var i in sel) {
        ...
        for (j in par) {
            ...
            if (!(n.key in ch)) {
                ch[n.key] = {
                    _title: n.data._title ? n.data._title : n.title,
                    ch: {}
                };
            }
...
var ctxt_menu_opts = {
    ...
    menu: [{
        ...
        children: [{
            ...
            {
                title: "Download",
                action: actionDataGet,
                cmd: "get"
            }, {
                title: "Upload",
                action: actionDataPut,
                cmd: "put"
            }, {
                title: "Provenance",
                action: actionDepGraph,
                cmd: "graph"
            }, {
                title: "Subscribe",
        ...
        children: [{
            title: "Data Record",
            action: actionNewData,
            cmd: "newd"
        }, {
            title: "Collection",
            action: actionNewColl,
            cmd: "newc"
        }, ]
    }, {
        title: "----"
    ...
    $("#data_tree", frame).fancytree({
        ...
        lazyLoad: function(event, data) {
            if (data.node.key == "mydata") {
                data.result = {
                    url: api.collView_url(my_root_key),
                    cache: false
                };
            } else if (data.node.key == "projects") {
                data.result = {
                    url: api.projList_url(true, true, true, model.SORT_TITLE, data.node.data.offset, settings.opts.page_sz),
                    cache: false
                };
                ...
                if (data.node.data.scope) {
                    ...
                    data.result = {
                        url: api.aclListSharedUsers_url(),
                        cache: false
                    };
                } else {
                    data.result = {
                        url: api.aclListSharedProjects_url(),
                        cache: false
                    };
                }
            } else if (data.node.key == "allocs") {
                data.result = {
                    url: api.repoAllocListByOwner_url(data.node.data.scope, data.node.data.scope),
                    cache: false
                };
            } else if (data.node.key.startsWith("repo/")) {
                data.result = {
                    url: api.repoAllocListItems_url(data.node.data.repo, data.node.data.scope, data.node.data.offset, settings.opts.page_sz),
                    cache: false
                };
                ...
                data.result = {
                    url: api.queryExec_url(data.node.key, data.node.data.offset, settings.opts.page_sz),
                    cache: false
                };
            } else if (data.node.key.startsWith("published")) {
                data.result = {
                    url: api.collListPublished_url(data.node.data.scope, data.node.data.offset, settings.opts.page_sz),
                    cache: false
                };
            } else {
