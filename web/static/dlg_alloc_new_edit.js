...
export function show( a_repo, a_alloc, a_excl, a_cb ){
    ...
    if ( a_alloc ){
        ...
        if ( a_alloc.id.startsWith("p/"))
            $("#subj_label",frame).html( "Project&nbspID:" );
        else
            $("#subj_label",frame).html( "User&nbspID:" );
            ...
            click: function() {
                if ( !a_alloc ){
                    ...
                    if ( !alloc.id.startsWith("u/") && !alloc.id.startsWith("p/")){
                        dialogs.dlgAlert("Data Entry Error","Invalid subject ID (must include 'u/' or 'p/' prefix.");
                        return;
                    }
                }
                ...
                if ( a_alloc ){
                    api.allocSet( a_repo, alloc.id, data_limit, rec_limit, function( ok, data ){
                        if ( ok ){
                            ...
                            dialogs.dlgAlert("Allocation Error","Allocation update failed ("+data+").");
                        }
                    });
                }else{
                    api.allocCreate( a_repo, alloc.id, data_limit, rec_limit, function( ok, data ){
                        if ( ok ){
                            a_cb( alloc );
                            dlg_inst.dialog('close');
                        }else{
                            dialogs.dlgAlert("Allocation Error","Allocation creation failed ("+data+").");
                        }
                    });
                }
