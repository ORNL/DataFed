...
export function show(a_mode, a_ids, a_cb) {
    ...
    frame.html(
        "<div class='ui-widget' style='height:95%'>" +
            ...
            (a_mode == model.TT_DATA_PUT
                ? "<br>File extension override: <input id='ext' type='text'></input><br>"
                : "") +
            (a_mode == model.TT_DATA_GET
                ? "<br><label for='orig_fname'>Download to original filename(s)</label><input id='orig_fname' type='checkbox'></input>"
                : "") +
            "</div></div></div>",
    );
    ...
    function updateGoBtn() {
        if (selection_ok && endpoint_ok) $("#go_btn").button("enable");
        else $("#go_btn").button("disable");
    }
        ...
        for (var i in a_ids) {
            ...
            if (sel) {
                src.push({
                    title:
                        item.id +
                        "&nbsp&nbsp&nbsp<span style='display:inline-block;width:9ch'>" +
                        info +
                        "</span>&nbsp" +
                        item.title,
                    selected: true,
        ...
        if (ep_list) {
            ...
            api.epView(ep, function (ok, data) {
                if (ok && !data.code) {
                    ...
                    path_in.val(
                        cur_ep.name + (cur_ep.default_directory ? cur_ep.default_directory : "/"),
                    );
    ...
    $("#browse", frame).on("click", function () {
        ...
        if (delim != -1) {
            ...
            if (path.charAt(path.length - 1) != "/") {
                ...
                if (delim > 0) path = path.substr(0, delim + 1);
                else path += "/";
            }
        } else path = cur_ep.default_directory ? cur_ep.default_directory : "/";
        dlgEpBrowse.show(
            cur_ep,
            path,
            a_mode == model.TT_DATA_GET ? "dir" : "file",
            function (sel) {
    ...
    $("#activate", frame).on("click", function () {
        window.open(
            "https://app.globus.org/file-manager?origin_id=" + encodeURIComponent(cur_ep.id),
            "",
        );
    });

    if (a_mode == model.TT_DATA_GET) $("#orig_fname", frame).checkboxradio();
    ...
    function updateEndpointOptions(cur_ep) {
        ...
        if (cur_ep.expires_in == -1) $("#activate", frame).button("disable");
        else $("#activate", frame).button("enable");
        ...
        if (!cur_ep || ep != cur_ep.name) {
            ...
            new Promise((resolve, reject) => {
                ...
                .then((data) => {
                    ...
                    var html =
                        "<option title='" +
                        (cur_ep.description ? util.escapeHTML(cur_ep.description) : "(no info)") +
                        "'>" +
                        util.escapeHTML(cur_ep.display_name || cur_ep.name) +
                        " (";

                    if (cur_ep.activated) html += Math.floor(cur_ep.expires_in / 3600) + " hrs";
                    else if (cur_ep.expires_in == -1) html += "active";
                    else html += "inactive";
                        ...
                        .then((data) => {
                            if (data.DATA && data.DATA.length) {
                                ...
                                for (var i in data.DATA) {
                                    ...
                                    html +=
                                        "<option title='" +
                                        util.escapeHTML(ep.description) +
                                        "'>" +
                                    ...
                                    if (!ep.activated && ep.expires_in == -1)
                                        ...
                                        html +=
                                            (ep.activated
                                                ? Math.floor(ep.expires_in / 3600) + " hrs"
                                                : "inactive") + ")</option>";
                                }
                                matches.html(html);
                                matches.prop("disabled", false);
            ...
            },
            {
                ...
                click: function () {
                    ...
                    if (a_mode == model.TT_DATA_GET || a_mode == model.TT_DATA_PUT) {
                        ...
                        var ids = [];
                        if (a_ids.length == 1) ids = [a_ids[0].id];
                        else {
                            var sel = rec_tree.getSelectedNodes();
                            for (var i in sel) {
                                ids.push(sel[i].key);
                        ...
                        api.xfrStart(
                            ...
                            function (ok, data) {
                                if (ok) {
                                    ...
                                    inst.dialog("close");
                                    util.setStatusText(
                                        "Task '" + data.task.id + "' created for data transfer.",
                                    );
                                    if (a_cb) {
                                        a_cb();
                                    }
