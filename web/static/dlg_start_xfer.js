...
export function show(a_mode, a_ids, a_cb) {
    ...
    if (!a_ids) {
        ...
        for (var i in a_ids) {
            ...
            if (sel) {
                src.push({
                    title: item.id + "&nbsp&nbsp&nbsp<span style='display:inline-block;width:9ch'>" + info + "</span>&nbsp" + item.title,
                    selected: true,
                    key: item.id
                });
            } else {
                skip += 1;
                src.push({
                    title: "<span style='color:#808080'>" + item.id + "&nbsp&nbsp&nbsp<span style='display:inline-block;width:9ch'>" + info + "</span>&nbsp" + item.title + "</span>",
                    unselectable: true,
                    key: item.id
                });
            }
