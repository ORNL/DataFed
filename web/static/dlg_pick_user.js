        ...
        lazyLoad: function(ev, data) {
            if (data.node.key == "collab") {
                ...
                if (srch_val.length > 1) {
                    data.result = {
                        url: api.userFindByName_url(srch_val, data.node.data.offset, settings.opts.page_sz),
                        cache: false
                    };
                } else {
                    data.result = [];
            ...
            if (a_data.node.key == "collab" || a_data.node.key == "all" || a_data.node.key == "search") {
                ...
                if (a_data.response.offset > 0 || a_data.response.total > (a_data.response.offset + a_data.response.count)) {
                    var pages = Math.ceil(a_data.response.total / settings.opts.page_sz),
                        page = 1 + a_data.response.offset / settings.opts.page_sz;
                    a_data.result.push({
                        title: "<button class='btn btn-icon-tiny''" + (page == 1 ? " disabled" : "") + " onclick='userPageLoad(\"" +
                ...
                for (i in a_data.response.user) {
                    ...
                    a_data.result.push({
                        title: (unsel ? "<span style='color:#808080'>" : "") + user.nameLast + ", " + user.nameFirst + " (" + user.uid.substr(2) + ")" + (unsel ? "</span>" : ""),
