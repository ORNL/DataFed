...
export function tooltipTheme(a_objs) {
    a_objs.tooltip({
        ...
        classes: {
            "ui-tooltip": "note ui-corner-all tooltip-style"
        },
        position: {
            my: "left+5 top+5",
            at: "left bottom",
...
export function buildObjSrcTree(obj, base, md_exp) {
    ...
    var src = [],
        k2, o, i, v, skip, val, vs, is_arr = Array.isArray(obj),
        fkey, kbase;
    ...
    Object.keys(obj).forEach(function(k) {
        ...
        if (!skip && (typeof obj[k] === 'object' && obj[k] !== null)) {
            ...
            if (k2.length + val.length > 60) {
                src.push({
                    key: fkey,
                    title: "<div class='md_tree_div'><div class='md_tree_key' title='" + fkey + "' draggable='true' ondragstart='md_key_drag(event)'>" + k2 + " :</div><div class='md_tree_val md_tree_val_indent'>" + val + "</div></div>",
                    icon: false
                });
            } else {
                src.push({
                    key: fkey,
                    title: "<span class='md_tree_div md_tree_key' title='" + fkey + "' draggable='true' ondragstart='md_key_drag(event)'>" + k2 + " : </span><span class='md_tree_div md_tree_val'>" + val + "</span>",
                    icon: false
                });
            }
        }
...
export function addTreePagingNode(a_data, a_no_last, a_pfx = "page") {
    if (a_data.response.offset > 0 || a_data.response.total > (a_data.response.offset + a_data.response.count)) {
        ...
        a_data.result.push({
            title: "<button class='btn btn-icon-tiny " + a_pfx + "-first''" + (page == 0 ? " disabled" : "") + "><span class='ui-icon ui-icon-triangle-1-w-stop'></span></button> \
            <button class='btn btn-icon-tiny " + a_pfx + "-prev'" + (page == 0 ? " disabled" : "") + "><span class='ui-icon ui-icon-triangle-1-w'></span></button> \
            Page " + (page + 1) + " of " + (page_mx + 1) +
                " <button class='btn btn-icon-tiny " + a_pfx + "-next'" + (page >= page_mx ? " disabled" : "") + "><span class='ui-icon ui-icon-triangle-1-e'></span></button>" +
                (a_no_last ? "" : ("<button class='btn btn-icon-tiny " + a_pfx + "-last'" + (page >= page_mx ? " disabled" : "") + " page='" + page_mx + "'><span class='ui-icon ui-icon-triangle-1-e-stop'></span></button>")),
            folder: false,
            icon: false,
            checkbox: false,
            hasBtn: true
