        ...
        const req = https.request(opts, (res) => {
            ...
            res.on('end', () => {
                ...
                if (res.statusCode >= 200 && res.statusCode < 300) {
                    ...
                    sendMessageDirect("UserFindByUUIDsRequest", "datafed-ws", {
...
app.get('/api/usr/find/by_uuids', (a_req, a_resp) => {
    sendMessage("UserFindByUUIDsRequest", {
        uuid: a_req.query.uuids
    }, a_req, a_resp, function(reply) {
...
app.get('/api/usr/view', (a_req, a_resp) => {
    ...
    sendMessage("UserViewRequest", {
        uid: a_req.query.id,
        details: (a_req.query.details == "true" ? true : false)
    }, a_req, a_resp, function(reply) {
        a_resp.json(reply.user[0]);
    });
});
...
app.get('/api/prj/delete', (a_req, a_resp) => {
    sendMessage("ProjectDeleteRequest", {
...
app.get('/api/prj/view', (a_req, a_resp) => {
    sendMessage("ProjectViewRequest", {
        id: a_req.query.id
    }, a_req, a_resp, function(reply) {
...
app.get('/api/grp/delete', (a_req, a_resp) => {
    sendMessage("GroupDeleteRequest", {
        uid: a_req.query.uid,
        gid: a_req.query.gid
    }, a_req, a_resp, function(reply) {
        a_resp.send(reply);
...
app.post('/api/query/create', (a_req, a_resp) => {
    ...
    sendMessage("QueryCreateRequest", {
...
app.get('/api/query/delete', (a_req, a_resp) => {
    sendMessage("QueryDeleteRequest", {
        id: JSON.parse(a_req.query.ids)
    }, a_req, a_resp, function(reply) {
        a_resp.send(reply);
    });
...
app.post('/api/dat/create/batch', (a_req, a_resp) => {
    ...
    sendMessage("RecordCreateBatchRequest", {
        records: a_req.body
    }, a_req, a_resp, function(reply) {
        a_resp.send(reply);
    });
...
app.post('/api/dat/update/batch', (a_req, a_resp) => {
    ...
    sendMessage("RecordUpdateBatchRequest", {
        records: a_req.body
    }, a_req, a_resp, function(reply) {
        a_resp.send(reply);
    });
...
app.get('/api/dat/lock', (a_req, a_resp) => {
    ...
    sendMessage("RecordLockRequest", {
        id: JSON.parse(a_req.query.ids),
        lock: a_req.query.lock == "true" ? true : false
    }, a_req, a_resp, function(reply) {
        a_resp.send(reply);
    });
});
...
app.get('/api/dat/lock/toggle', (a_req, a_resp) => {
    sendMessage("RecordLockToggleRequest", {
        id: a_req.query.id
    }, a_req, a_resp, function(reply) {
        a_resp.send(reply);
    });
...
app.get('/api/dat/delete', (a_req, a_resp) => {
    ...
    sendMessage("RecordDeleteRequest", {
        id: JSON.parse(a_req.query.ids)
    }, a_req, a_resp, function(reply) {
        a_resp.send(reply);
    });
...
app.get('/api/dat/export', (a_req, a_resp) => {
    sendMessage("RecordExportRequest", {
        id: JSON.parse(a_req.query.ids)
    }, a_req, a_resp, function(reply) {
        a_resp.send(reply);
    });
...
app.get('/api/dat/dep/get', (a_req, a_resp) => {
    sendMessage("RecordGetDependenciesRequest", {
        id: a_req.query.ids
    }, a_req, a_resp, function(reply) {
        a_resp.send(reply);
    });
...
app.get('/api/dat/dep/graph/get', (a_req, a_resp) => {
    sendMessage("RecordGetDependencyGraphRequest", {
        id: a_req.query.id
    }, a_req, a_resp, function(reply) {
        a_resp.send(reply);
    });
...
app.get('/api/acl/update', (a_req, a_resp) => {
    sendMessage("ACLUpdateRequest", {
        id: a_req.query.id,
        rules: a_req.query.rules
    }, a_req, a_resp, function(reply) {
...
app.get('/api/acl/shared/list', (a_req, a_resp) => {
    sendMessage("ACLSharedListRequest", {
...
app.get('/api/acl/shared/list/items', (a_req, a_resp) => {
    sendMessage("ACLSharedListItemsRequest", {
...
app.get('/api/note/list/by_subject', (a_req, a_resp) => {
    sendMessage("NoteListBySubjectRequest", {
        subject: a_req.query.subject
    }, a_req, a_resp, function(reply) {
        a_resp.send(reply);
    });
...
app.get('/api/task/view', (a_req, a_resp) => {
    ...
    sendMessage("TaskViewRequest", {
        "taskId": a_req.query.id
    }, a_req, a_resp, function(reply) {
        a_resp.json(reply);
    });
});
...
app.get('/api/col/get_parents', (a_req, a_resp) => {
    sendMessage("CollGetParentsRequest", {
        id: a_req.query.id
    }, a_req, a_resp, function(reply) {
...
app.get('/api/col/get_offset', (a_req, a_resp) => {
    sendMessage("CollGetOffsetRequest", {
        id: a_req.query.id,
        item: a_req.query.item_id,
        pageSz: a_req.query.page_sz
    }, a_req, a_resp, function(reply) {
...
app.get('/api/col/link', (a_req, a_resp) => {
    sendMessage("CollWriteRequest", {
        id: a_req.query.coll,
        add: JSON.parse(a_req.query.items)
    }, a_req, a_resp, function(reply) {
        a_resp.send(reply);
    });
});
...
app.get('/api/col/unlink', (a_req, a_resp) => {
    sendMessage("CollWriteRequest", {
        id: a_req.query.coll,
        rem: JSON.parse(a_req.query.items)
    }, a_req, a_resp, function(reply) {
        a_resp.send(reply);
    });
});
...
app.get('/api/repo/calc_size', (a_req, a_resp) => {
    sendMessage("RepoCalcSizeRequest", {
...
app.get('/api/repo/alloc/list/by_repo', (a_req, a_resp) => {
    sendMessage("RepoListAllocationsRequest", {
        id: a_req.query.id
    }, a_req, a_resp, function(reply) {
...
app.get('/api/repo/alloc/list/by_object', (a_req, a_resp) => {
    sendMessage("RepoListObjectAllocationsRequest", {
        id: a_req.query.id
    }, a_req, a_resp, function(reply) {
        a_resp.json(reply.alloc ? reply.alloc : []);
    });
...
app.get('/api/repo/alloc/view', (a_req, a_resp) => {
    sendMessage("RepoViewAllocationRequest", {
        repo: a_req.query.repo,
        subject: a_req.query.subject
    }, a_req, a_resp, function(reply) {
        a_resp.send(reply);
...
app.get('/api/repo/alloc/stats', (a_req, a_resp) => {
    sendMessage("RepoAllocationStatsRequest", {
        repo: a_req.query.repo,
        subject: a_req.query.subject
    }, a_req, a_resp, function(reply) {
        a_resp.json(reply.alloc ? reply.alloc : {});
...
app.get('/api/repo/alloc/create', (a_req, a_resp) => {
    sendMessage("RepoAllocationCreateRequest", {
        repo: a_req.query.repo,
        subject: a_req.query.subject,
...
app.get('/api/repo/alloc/delete', (a_req, a_resp) => {
    sendMessage("RepoAllocationDeleteRequest", {
        repo: a_req.query.repo,
        subject: a_req.query.subject
    }, a_req, a_resp, function(reply) {
        a_resp.send(reply);
...
app.get('/api/repo/alloc/set', (a_req, a_resp) => {
    sendMessage("RepoAllocationSetRequest", {
        repo: a_req.query.repo,
        subject: a_req.query.subject,
        dataLimit: a_req.query.data_limit,
        recLimit: a_req.query.rec_limit
...
app.get('/api/top/search', (a_req, a_resp) => {
    sendMessage("TopicSearchRequest", {
...
app.get('/api/sch/view', (a_req, a_resp) => {
    sendMessage("SchemaViewRequest", {
        id: a_req.query.id,
        resolve: a_req.query.resolve
    }, a_req, a_resp, function(reply) {
        a_resp.json(reply);
    });
});
...
app.post('/api/sch/delete', (a_req, a_resp) => {
    sendMessage("SchemaDeleteRequest", {
        id: a_req.query.id
    }, a_req, a_resp, function(reply) {
        a_resp.json(reply);
    });
...
function setAccessToken(a_uid, a_acc_tok, a_ref_tok, a_expires_sec) {
    ...
    sendMessageDirect("UserSetAccessTokenRequest", a_uid, {
        access: a_acc_tok,
        refresh: a_ref_tok,
        expiresIn: a_expires_sec
    }, function(reply) {
