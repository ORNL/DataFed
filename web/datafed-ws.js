...
function startServer() {
  ...
  logger.info(
    startServer.name,
    getCurrentLineNumber(),
    `External URL: ${g_extern_url}`,
  );
  logger.info(
    startServer.name,
    getCurrentLineNumber(),
    `Core server addr: ${g_core_serv_addr}`,
  );
  ...
  sendMessageDirect("VersionRequest", "", {}, function (reply) {
    if (!reply) {
      ...
      g_oauth_credentials = {
        ...
        scopes:
          "urn:globus:auth:scope:transfer.api.globus.org:all offline_access openid",
      };
...
app.get("/ui/welcome", (a_req, a_resp) => {
  if (a_req.session.uid && a_req.session.reg) a_resp.redirect("/ui/main");
    ...
    a_resp.setHeader(
      ...
      `script-src 'nonce-${nonce}'  auth.globus.org`,
    );
    a_resp.render("index", {
...
app.get("/ui/authn", (a_req, a_resp) => {
  logger.info(
    "/ui/authn",
    getCurrentLineNumber(),
    "Globus authenticated - log in to DataFed",
  );
  ...
  g_globus_auth.code.getToken(a_req.originalUrl).then(
    function (client_token) {
      ...
      const opts = {
        ...
        auth:
          g_oauth_credentials.clientId + ":" + g_oauth_credentials.clientSecret,
        headers: {
      ...
      req.write(
        "token=" + client_token.accessToken + "&include=identities_set",
      );
      req.end();
    },
    function (reason) {
