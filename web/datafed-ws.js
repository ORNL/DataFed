...
app.get('/ui/welcome', (a_req, a_resp) => {
    if ( a_req.session.uid && a_req.session.reg )
        ...
        a_resp.render('index',{nonce:a_resp.locals.nonce, theme:theme,version:g_version,test_mode:g_test,...g_ga});
    }
});
...
app.get('/ui/main', (a_req, a_resp) => {
    if ( a_req.session.uid && a_req.session.reg ){
        ...
        a_resp.render('main',{nonce:a_resp.locals.nonce,user_uid:a_req.session.uid,theme:theme,version:g_version,test_mode:g_test,...g_ga});
    }else{
...
app.get('/ui/register', (a_req, a_resp) => {
    ...
    if ( !a_req.session.uid ){
        ...
        a_resp.locals.nonce = nonce;
        a_resp.setHeader('Content-Security-Policy', `script-src 'nonce-${nonce}' auth.globus.org`);
        a_resp.render('register', {nonce:a_resp.locals.nonce, uid: a_req.session.uid, uname: clean, theme: theme, version: g_version, test_mode: g_test, ...g_ga });
    }
});
...
app.get('/ui/error', (a_req, a_resp) => {
    ...
    a_resp.locals.nonce = nonce;
    a_resp.setHeader('Content-Security-Policy', `script-src 'nonce-${nonce}'`);
    a_resp.render('error',{nonce:a_resp.locals.nonce,theme:"light",version:g_version,test_mode:g_test,...g_ga});
});
...
function loadSettings(){
    ...
    try{
        ...
        if ( config.operations ){
          g_ga = { enableGoogleAnalytics: config.operations.google_analytics_tag !== '', googleAnalyticsTag: config.operations.google_analytics_tag };
        }
    }catch( e ){
