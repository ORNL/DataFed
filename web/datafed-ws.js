...
const app = express();
const MAX_CTX = 50;
var g_host, g_port, g_server_key_file, g_server_cert_file, g_server_chain_file, g_system_secret, g_session_secret, g_test, g_msg_by_id = {}, g_msg_by_name = {}, g_core_sock = zmq.socket("dealer"), g_core_serv_addr, g_globus_auth, g_extern_url, g_oauth_credentials, g_ctx = new Array(MAX_CTX), g_ctx_next = 0, g_client_id, g_client_secret, g_ready_start = 4, g_version, g_ver_release_year, g_ver_release_month, g_ver_release_day, g_ver_release_hour, g_ver_release_minute, g_ver_api_major, g_ver_api_minor, g_ver_api_patch, g_tls, g_google_analytics;
...
function startServer() {
    ...
    sendMessageDirect("VersionRequest", "", {}, function (reply) {
        if (!reply) {
        ...
        else if (reply.api_major != g_ver_api_major ||
            reply.api_minor < g_ver_api_minor ||
            reply.api_minor > g_ver_api_minor + 9) {
            logger.error(startServer.name, getCurrentLineNumber(), "ERROR: Incompatible api version detected (" +
                reply.api_major +
                "." +
                reply.api_minor +
                "." +
            ...
            var warning_msg = "WARNING: A newer web server may be available the latest release version is: (" +
                reply.release_year +
                "." +
                reply.release_month +
                "." +
...
app.get("/ui/register", (a_req, a_resp) => {
    ...
    if (!a_req.session.uid) {
        ...
        logger.info("/ui/register", getCurrentLineNumber(), " - registration access (" +
            a_req.session.uid +
            ") from " +
...
app.get("/ui/logout", (a_req, a_resp) => {
    ...
    a_req.session.destroy(function () {
        ...
        a_resp.redirect("https://auth.globus.org/v2/web/logout?redirect_name=DataFed&redirect_uri=" +
            g_extern_url);
    });
});
        ...
        const req = https.request(opts, (res) => {
            ...
            res.on("end", () => {
                if (res.statusCode >= 200 && res.statusCode < 300) {
                    var userinfo = JSON.parse(data), uid = userinfo.username.substr(0, userinfo.username.indexOf("@"));
                    ...
                    sendMessageDirect("UserFindByUUIDsRequest", "datafed-ws", { uuid: userinfo.identities_set }, function (reply) {
                        if (!reply) {
                            ...
                            logger.info("/ui/authn", getCurrentLineNumber(), "User: " +
                                uid +
                                " verified, acc:" +
...
app.get("/api/acl/update", (a_req, a_resp) => {
    sendMessage("ACLUpdateRequest", { id: a_req.query.id, rules: a_req.query.rules }, a_req, a_resp, function (reply) {
        if (reply.rule && reply.rule.length) {
            logger.debug("/api/acl/update", getCurrentLineNumber(), "User: " +
                a_req.session.uid +
                " - ACL update, id: " +
                a_req.query.id +
                " " +
...
app.get("/ui/ep/autocomp", (a_req, a_resp) => {
    sendMessage("UserGetAccessTokenRequest", {}, a_req, a_resp, function (reply) {
        const opts = {
            hostname: "transfer.api.globusonline.org",
            method: "GET",
            path: "/v0.10/endpoint_search?filter_scope=all&fields=display_name,canonical_name,id,description,organization,activated,expires_in,default_directory&filter_fulltext=" +
...
function sendMessage(a_msg_name, a_msg_data, a_req, a_resp, a_cb, a_anon) {
    ...
    allocRequestContext(a_resp, function (ctx) {
        ...
        if (msg_buf.length) {
            ...
            logger.debug(sendMessage.name, getCurrentLineNumber(), "MsgType is: " +
                msg._msg_type +
                " Writing ctx to frame, " +
                ctx +
                " buffer size " +
                msg_buf.length, corr_id);
        }
        else {
            g_core_sock.send("BEGIN_DATAFED", zmq.ZMQ_SNDMORE);
            g_core_sock.send(route_count, zmq.ZMQ_SNDMORE);
...
function sendMessageDirect(a_msg_name, a_client, a_msg_data, a_cb) {
    ...
    allocRequestContext(null, function (ctx) {
        ...
        if (msg_buf.length) {
            ...
            g_core_sock.send(frame, zmq.ZMQ_SNDMORE);
            g_core_sock.send(msg_buf);
            logger.debug(sendMessageDirect.name, getCurrentLineNumber(), "MsgType is: " +
                msg._msg_type +
...
g_core_sock.on("message", function (delim, header, route_count, delim2, correlation_id, key, id, frame, msg_buf) {
    ...
    if (msg_class) {
        ...
        if (msg_buf && msg_buf.length) {
            try {
                ...
                if (!msg) {
                    logger.error("g_core_sock.on", getCurrentLineNumber(), "ERROR: msg decode failed: no reason, correlation_id: " +
                        correlation_id);
                }
            }
            catch (err) {
    ...
    if (f) {
        ...
        g_ctx[ctx] = null;
        logger.error("g_core_sock.on", getCurrentLineNumber(), "ERROR: no callback found for ctxt: " +
            ctx +
            " - msg type: " +
            mtype +
            ", name: " +
