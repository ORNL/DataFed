...
if (!isLocal) {
  ...
  rules.push({
    ...
    use: [
      ...
      {
        loader: "sass-loader",
        options: { sassOptions: { includePaths: sassIncludePaths } },
      },
    ],
  });
} else {
...
module.exports = {
  ...
  devServer: {
    ...
    host: "0.0.0.0",
    client: {
      // Added client configuration for HMR
      overlay: {
        errors: true,
        warnings: false,
      },
    ...
    devMiddleware: {
      // Standard way to define publicPath for devServer
      publicPath: "/", // Should match output.publicPath
    },
    stats: {
      // Deprecated, use infrastructureLogging in webpack-dev-server v4+ for similar control
      assets: true,
      children: false,
      chunks: false,
      hash: false,
