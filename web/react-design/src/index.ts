...
if (isProduction && internalKey && internalCert) {
    ...
    log.warn("SSL Key or Certificate not provided for production. HTTPS will not be enabled.");
}
...
if (isDevelopment) {
    let webpackDevMiddleware: typeof import("webpack-dev-middleware").default;
    let webpackHotMiddleware: typeof import("webpack-hot-middleware").default;
    ...
    try {
        ...
        if (!webpackConfig.output?.publicPath) {
            log.error("Webpack configuration must have \"output.publicPath\" defined for dev server.");
            process.exit(1);
        }

    } catch (e) {
        log.error("Webpack dev dependencies or config not found. Make sure to install them for development.", e);
        process.exit(1);
    }
...
const gracefulShutdown = (serverInstance: http.Server | https.Server, signal: string) => {
...
const startServer = (): void => {
    ...
    process.on("unhandledRejection", (reason: any, promise: Promise<any>) => {
        log.error("Unhandled Rejection at:", promise, "reason:", reason instanceof Error ? reason.stack : reason);
    });
};
