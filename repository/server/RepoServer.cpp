...
// m_msg_handlers[m_msg_mapper->getMessageType( proto_id, #msg )] = func #define
// SET_MSG_HANDLER(proto_id,msg,func)  m_msg_handlers[MsgBuf::findMessageType(
// proto_id, #msg )] = func

Server::Server() : m_config(Config::getInstance()) {
...
void Server::run() {
  ...
  for (uint16_t t = 0; t < m_config.num_req_worker_threads; ++t) {
    std::cout << "Creating worker " << t << " out of "
              << m_config.num_req_worker_threads << std::endl;
...
void Server::checkServerVersion() {
  ...
  //    EXCEPT_PARAM( 1, "Temp security key generation failed: " <<
  //    zmq_strerror( errno ));
  ...
  for (int i = 0; i < 10; i++) {
    ...
    if (response.time_out) {
      DL_ERROR("Timeout waiting for response from core server: "
      ...
      if (ver_reply->api_major() != SDMS::repository::version::MAJOR) {
        EXCEPT_PARAM(1,
                     "Incompatible messaging api detected major backwards "
      ...
      if (ver_reply->api_minor() + 9 > SDMS::repository::version::MINOR) {
        DL_WARN("Significant changes in message api detected ("
                << ver_reply->api_major() << "." << ver_reply->api_minor()
                << "." << ver_reply->api_patch() << ")");
      }
...
void Server::ioSecure() {
  ...
  try {
    ...
    {  // Proxy Server Credentials and Socket Options - these options are used
      ...
      std::cout << "PRIVATE KEY for repo secure connection to core server "
    ...
    // incoming_operators.push_back(
    // oper_factory.create(OperatorType::RouterBookKeeping, router_id_to_add) );
    std::cout << __LINE__ << std::endl;
    ...
    // ,socket_options, socket_credentials, std::move(incoming_operators));
    std::cout << __LINE__
              << " Created proxy target addresses are:" << std::endl;
    ...
    // MsgComm frontend( "tcp://*:" + to_string(m_config.port), MsgComm::ROUTER,
    // true, &m_config.sec_ctx ); MsgComm backend( "inproc://workers",
    // MsgComm::DEALER, true );
