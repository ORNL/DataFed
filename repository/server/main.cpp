...
int main(int a_argc, char **a_argv) {
  ...
  try {
    DL_INFO(log_context, "DataFed repo server starting, ver "
                             << repository::version::MAJOR << "."
                             << repository::version::MINOR << "."
    ...
    opts.add_options()("help,?", "Show help")(
        ...
        "port,p", po::value<uint16_t>(&config.port),
        "Service port")("server,s", po::value<string>(&config.core_server),
                        "Core server address")(
        "globus-collection-path,g",
        po::value<string>(&config.globus_collection_path),
        "Path to Globus collection default value is /mnt/datafed-repo")(
        "threads,t", po::value<uint32_t>(&config.num_req_worker_threads),
        "Number of worker threads")("cfg", po::value<string>(&cfg_file),
    ...
    try {
      ...
      if (opt_map.count("help")) {
        cout << "DataFed Repo Server, ver. " << repository::version::MAJOR
             << "." << repository::version::MINOR << "."
             << repository::version::PATCH << "\n";
      ...
      if (config.globus_collection_path.size() &&
          config.globus_collection_path.back() != '/') {
