...
BOOST_AUTO_TEST_CASE(test_gsi_authz_init) {
    ...
    if (!config_file) {
        throw std::ios_base::failure("Failed to create the configuration file. " + file_path);
    }
    ...
    config_file << "globus-collection-path=" << globus_collection_path << std::endl;
    ...
    if (!repo_pub) {
        throw std::ios_base::failure("Failed to create the configuration file. " + pub_key_path);
    }
    ...
    if (!repo_priv) {
        throw std::ios_base::failure("Failed to create the configuration file. " + priv_key_path);
    }
    ...
    server_file.close(); // Close the file
        ...
        throw std::runtime_error("Failed to set environment variable: " + conf_env_name);
    }
