...
void setEnvVar() {
    ...
    if (setenv(config_file_path_key, config_file_path_value, 1) == 0) {
        std::cout << config_file_path_key << " is set to " << config_file_path_value << std::endl;
    } else {
...
void createKeyFile(std::string file_path, std::string content) {
    ...
    if (!file_stream.is_open()) {
        std::cerr << "Failed to open or create the file: " << file_path << std::endl;
        return;
    }
...
static const std::string server_key = "^-j(*ypiy?Nt9^K.c3GRS?4)j?4a:O.%yV=2+qLs";
...
void createFiles() {
    ...
    if (!configFile.is_open()) {
        std::cerr << "Failed to open or create the file: " << cfg_file_path << std::endl;
        return;
    }
