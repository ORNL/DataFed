...
BOOST_AUTO_TEST_CASE(ValidUuidConversion) {
    unsigned char uuid[16] = {0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef, 0x12, 0x34, 0x56, 0x78, 0x90, 0xab, 0xcd, 0xef};
    char output[37]; // UUID string length + null terminator
    uuidToStr(uuid, output);

    BOOST_CHECK_EQUAL(std::string(output), "12345678-90ab-cdef-1234-567890abcdef");
}
...
BOOST_AUTO_TEST_CASE(EmptyUuid) {
    ...
    BOOST_CHECK_EQUAL(std::string(output), "00000000-0000-0000-0000-000000000000");
}

// uuidToStr Max UUID
BOOST_AUTO_TEST_CASE(MaxUuid) {
    unsigned char uuid[16] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
    char output[37];
    uuidToStr(uuid, output);

    BOOST_CHECK_EQUAL(std::string(output), "ffffffff-ffff-ffff-ffff-ffffffffffff");
}

// uuidToStr Random UUID
BOOST_AUTO_TEST_CASE(RandomUuid) {
    unsigned char uuid[16] = {0xde, 0xad, 0xbe, 0xef, 0xca, 0xfe, 0xba, 0xbe, 0xde, 0xad, 0xbe, 0xef, 0xca, 0xfe, 0xba, 0xbe};
    char output[37];
    uuidToStr(uuid, output);

    BOOST_CHECK_EQUAL(std::string(output), "deadbeef-cafe-babe-dead-beefcafebabe");
}
