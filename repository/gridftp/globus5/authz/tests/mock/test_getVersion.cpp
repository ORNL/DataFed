...
BOOST_AUTO_TEST_CASE(mock_liveness_test_get_version) {
  ...
  // Public and private keys are consistent with what is in the mock for the repo
  cred_options[CredentialType::PUBLIC_KEY] = "Wxwm^-Cf7cJrqS)}/B?cDAq(L=@AwSA*({jhBu1]";
  cred_options[CredentialType::PRIVATE_KEY] = "1:BoDR8-#sGZuRWyP*jr+Csm-kT}zV:tn@gRDS.u";
  // Server key can be generated by the mock
  cred_options[CredentialType::SERVER_KEY] = server_key;

  std::cout << "cred public key: " << cred_options[CredentialType::PUBLIC_KEY] << std::endl;
  std::cout << "cred private key: " << cred_options[CredentialType::PRIVATE_KEY] << std::endl;
  // Server key should be the public key of the mock-datafed-core.pub
  std::cout << "cred server key: " << cred_options[CredentialType::SERVER_KEY] << std::endl;

  CredentialFactory cred_factory;
  std::unique_ptr<ICredentials> sec_ctx = cred_factory.create(ProtocolType::ZQTP, cred_options);
  ...
  auto client_comm = [&](const std::string &socket_id, const std::string &address,
    ...
    std::cout << "Port: " << std::to_string(socket_options.port.value()) << std::endl;
    ...
    corr_id = std::get<std::string>(msg_from_client->get(MessageAttribute::CORRELATION_ID));
    ...
    msg_from_client->set(MessageAttribute::KEY, cred_options[CredentialType::PUBLIC_KEY]);
  ...
  { // Receive a version response payload
    ...
    if ( response.message ) {
      ...
      BOOST_CHECK_EQUAL(corr_id, std::get<std::string>(response.message->get(MessageAttribute::CORRELATION_ID)));
      ...
      Anon::VersionReply * version_response = dynamic_cast<Anon::VersionReply *>(google_msg_ptr);
