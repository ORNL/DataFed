...
gss_ctx_id_t findContext(globus_gsi_authz_handle_t a_handle) {
  ...
  for (; c != e; ++c) {
    if (c->handle == a_handle) return c->context;
  }
...
bool loadConfig() {
  ...
  if (!cfg_file) {
    ...
    if (!inf) {
      syslog(LOG_ERR,
             "DataFed - DATAFED_AUTHZ_CFG_FILE env variable not set, and "
             "datafed-authz.cfg is not located in default location "
             "/opt/datafed/authz");
      return true;
    }
  } else {
  ...
  if (inf) {
    ...
    while (1) {
      ...
      // Stop at EOF
      if (!fgets(buf, MAX_BUF, inf)) break;
      ...
      if (!val) {
        syslog(LOG_ERR,
               "DataFed - Syntax error in authz config file at line %i.", lc);
        return true;
      } else {
        *val = 0;
        val++;
    ...
    if (g_config.user[0] == 0) strcat(miss, " user");
    if (g_config.repo_id[0] == 0) strcat(miss, " repo_id");
    if (g_config.server_addr[0] == 0) strcat(miss, " server_address");
    if (g_config.pub_key[0] == 0) strcat(miss, " pub_key");
    if (g_config.globus_collection_path[0] == 0)
      strcat(miss, " globus-collection-path");
    if (g_config.priv_key[0] == 0) strcat(miss, " priv_key");
    if (g_config.server_key[0] == 0) strcat(miss, " server_key");
...
globus_result_t gsi_authz_init() {
  ...
  syslog(LOG_INFO, "                     Release, version %s",
         getReleaseVersion());
  memset(g_active_contexts, 0, sizeof(g_active_contexts));

  // This line is different
  if (loadConfig()) return GLOBUS_FAILURE;
...
globus_result_t gsi_authz_authorize_async(va_list ap) {
  ...
  syslog(LOG_ERR, "gsi_authz_authorize_async, handle: %p, act: %s, obj: %s",
  ...
  if (context != 0) {
    ...
    if (maj_stat == GSS_S_COMPLETE) {
      ...
      if (maj_stat == GSS_S_COMPLETE) {
        ...
        if (maj_stat == GSS_S_COMPLETE) {
          syslog(LOG_INFO, "Auth client: %s, file: %s, action: %s",
...
// Testing hack
          ...
          if (strncmp((char *)client_buf.value,
            ...
            syslog(LOG_ERR, "Invalid certificate subject prefix: %s",
            ...
            if (strncmp((char *)client_buf.value + 52, "u_", 2) == 0) {
              ...
              if (!decodeUUID((char *)client_buf.value + 54, client_id)) {
                syslog(LOG_ERR, "Failed to decode subject UUID: %s",
          ...
          syslog(LOG_ERR, "gss_display_name target FAILED, maj: %d, min: %d",
                 maj_stat, min_stat);
        }

        gss_release_buffer(&min_stat, &client_buf);
      } else {
        syslog(LOG_ERR, "gss_display_name source FAILED, maj: %d, min: %d",
               maj_stat, min_stat);
      }
    } else {
      syslog(LOG_ERR, "gss_inquire_context FAILED, maj: %d, min: %d", maj_stat,
             min_stat);
    }
  } else {
