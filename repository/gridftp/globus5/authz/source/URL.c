...
int ftpExtractRelativePath(const char *url, char *relativePath, size_t maxLength) {
  ...
  if( len_of_url == len_of_prefix ) {
      ...
      fprintf(stderr, "Error: Invalid URL format expected ftp://domain/ instead received %s\n", url);
      return 0;
      ...
      fprintf(stderr, "Error: Invalid URL format missing domain name expected ftp://domain/ instead received %s\n", url);
      return 0;
  }
  ...
  printf("slashPtr is %s\n", slashPtr);
	...
	if (pathLength >= maxLength) {
		fprintf(stderr, "Error: Insufficient buffer size max size is %ld actual size is %ld\n", maxLength, pathLength);
		return 0;
	}
