...
namespace Core {
  ...
  std::vector<std::string> AuthMap::getExpiredKeys(const PublicKeyType pub_key_type, const time_t threshold) const noexcept {

    auto expiredKeys = [=](const AuthMap::client_map_t & client_map, const time_t expire_time) -> std::vector<std::string> {
