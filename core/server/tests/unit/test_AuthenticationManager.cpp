...
BOOST_AUTO_TEST_CASE( testing_AuthenticationManagerPurgeTrans ) {
  ...
  auto promote_condition = std::make_unique<Promote>(
      promote_after_access_attemps,
      promote_from,
      promote_to
      ); 

  purge_conditions[PublicKeyType::TRANSIENT].push_back(std::move(promote_condition));
...
BOOST_AUTO_TEST_CASE( testing_AuthenticationManagerSessionReset ) {
  ...
  auto reset_condition = std::make_unique<Reset>(
      reset_after_access_attempts,
      apply_reset_to
      ); 

  purge_conditions[PublicKeyType::SESSION].push_back(std::move(reset_condition)); 
