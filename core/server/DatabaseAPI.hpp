...
class DatabaseAPI {
 public:
  ...
  void checkPerms(const Auth::CheckPermsRequest &a_request,
                  Auth::CheckPermsReply &a_reply);
  void getPerms(const Auth::GetPermsRequest &a_request,
                Auth::GetPermsReply &a_reply);
  ...
  // void recordUpdatePostPut( const std::string & a_data_id, size_t
  // a_file_size, time_t a_mod_time, const std::string & a_src_path, const
  // std::string * a_ext = 0 );
  ...
  void recordLock(const Auth::RecordLockRequest &a_request,
                  Auth::ListingReply &a_reply);
  // void recordSearch( const Auth::RecordSearchRequest & a_request,
  // Auth::ListingReply & a_reply ); void recordSearchPublished( const
  // Auth::RecordSearchPublishedRequest & a_request, Auth::ListingReply &
  // a_reply
  // );
  void recordListByAlloc(const Auth::RecordListByAllocRequest &a_request,
                         Auth::ListingReply &a_reply);
  // void recordGetDependencies( const Auth::RecordGetDependenciesRequest &
  // a_request, Auth::ListingReply & a_reply );
  void recordGetDependencyGraph(
      const Auth::RecordGetDependencyGraphRequest &a_request,
      Auth::ListingReply &a_reply);
  // void recordUpdateDataMoveInit( const libjson::Value & a_rec_ids, const
  // std::string & a_new_repo_id, const std::string & a_new_owner_id, const
  // std::string & a_new_coll_id ); void recordUpdateDataMoveRevert( const
  // libjson::Value & a_rec_ids ); void recordUpdateDataMoveFinalize( const
  // libjson::Value & a_rec_ids );

  void generalSearch(const Auth::SearchRequest &a_request,
                     Auth::ListingReply &a_reply);

  // void doiView( const Auth::DOIViewRequest & a_request, Auth::RecordDataReply
  // & a_reply );
  ...
  void collWrite(const Auth::CollWriteRequest &a_request,
                 Auth::ListingReply &a_reply);
  void collMove(const Auth::CollMoveRequest &a_request,
                Anon::AckReply &a_reply);
  ...
  // void catalogSearch( const Auth::CatalogSearchRequest & a_request,
  // Auth::CatalogSearchReply & a_reply );
  ...
  // void aclByUser( const Auth::ACLByUserRequest & a_request,
  // Auth::UserDataReply & a_reply ); void aclByUserList( const
  // Auth::ACLByUserListRequest & a_request,  Auth::ListingReply & a_reply );
  // void aclByProj( const Auth::ACLByProjRequest & a_request,
  // Auth::ProjectDataReply & a_reply ); void aclByProjList( const
  // Auth::ACLByProjListRequest & a_request,  Auth::ListingReply & a_reply );
  ...
  void topicSearch(const Auth::TopicSearchRequest &a_request,
                   Auth::TopicDataReply &a_reply);
  // void topicListCollections( const Auth::TopicListCollectionsRequest &
  // a_request, Auth::TopicListCollectionsReply & a_reply ); void topicLink(
  // const Auth::TopicLinkRequest & a_request, Anon::AckReply & a_reply ); void
  // topicUnlink( const Auth::TopicUnlinkRequest & a_request, Anon::AckReply &
  // a_reply );
  ...
  void taskRun(const std::string &a_task_id, libjson::Value &a_task_reply,
               int *a_step = 0, std::string *a_err_msg = 0);
  ...
  void taskUpdate(const std::string &a_id, TaskStatus *a_status = 0,
                  const std::string *a_message = 0, double *a_progress = 0,
                  libjson::Value *a_state = 0);
 ...
 private:
  long dbGet(const char *a_url_path,
             ...
             libjson::Value &a_result, bool a_log = true);
  bool dbGetRaw(
  ...
  void setDataPutReply(Auth::DataPutReply &a_reply,
                       const libjson::Value &a_result);
  // void setCatalogSearchReply( Auth::CatalogSearchReply & a_reply, const
  // libjson::Value & a_result ); void setCatItemInfoData( CatItemInfoData *
  // a_item, const libjson::Value::Object & a_obj );
  ...
  // uint32_t parseCatalogSearchRequest( const Auth::CatalogSearchRequest &
  // a_request, std::string & a_query, std::string & a_params, bool a_partial =
  // false ); void parseRecordSearchPublishedRequest( const
  // Auth::RecordSearchPublishedRequest & a_request, std::string & a_query,
  // std::string & a_params );
