...
void DatabaseAPI::recordUpdateSize(const Auth::RepoDataSizeReply &a_size_rep,
  ...
  for (int i = 0; i < a_size_rep.size_size(); i++) { // size size :D
    ...
    record_entry["size"] = to_string(a_size_rep.size(i).size()); // size size size!! :D
...
void DatabaseAPI::collCreate(const Auth::CollCreateRequest &a_request,
  ...
  */
  ...
  void DatabaseAPI::setRepoData(
      Auth::RepoDataReply * a_reply, std::vector<RepoData> & a_repos,
  ...
  void DatabaseAPI::topicListTopics(
      const Auth::TopicListTopicsRequest &a_request,
  ...
  void DatabaseAPI::noteCommentEdit(
      const Auth::NoteCommentEditRequest &a_request,
  ...
  */
  ...
  void DatabaseAPI::taskRun(
      const std::string &a_task_id, libjson::Value &a_task_reply,
      LogContext log_context, int *a_step, std::string *a_err_msg) {
    vector<pair<string, string>> params;
  ...
  void DatabaseAPI::taskAbort(
      const std::string &a_task_id, const std::string &a_msg,
  ...
  void DatabaseAPI::taskInitDataGet(
      const Auth::DataGetRequest &a_request, Auth::DataGetReply &a_reply,
      libjson::Value &a_result, LogContext log_context) {
    // TODO: json serialization
    // string body = "{\"id\":[";
    nlohmann::json payload;
  ...
  void DatabaseAPI::taskInitDataPut(
      const Auth::DataPutRequest &a_request, Auth::DataPutReply &a_reply,
      libjson::Value &a_result, LogContext log_context) {
    nlohmann::json payload;
    // string body = "{\"id\":[\"" + a_request.id() + "\"]";
    payload["id"] =
  ...
  void DatabaseAPI::taskFinalize(
      const std::string &a_task_id, bool a_succeeded, const std::string &a_msg,
      libjson::Value &a_result, LogContext log_context) {
    vector<pair<string, string>> params;
    params.push_back({"task_id", a_task_id});
  ...
  uint32_t DatabaseAPI::parseSearchRequest(
      const Auth::SearchRequest &a_request, std::string &a_qry_begin,
      std::string &a_qry_end, std::string &a_qry_filter, std::string &a_params,
      LogContext log_context) {
    ...
    if (a_request.has_text() > 0) {
      a_qry_begin += " and analyzer(" +
                     parseSearchTextPhrase(a_request.text(), "i") +
                     ",'text_en')";
    }
    ...
    */
    ...
    for (boost::tokenizer<boost::escaped_list_separator<char>>::iterator t =
      ...
      if (pos != string::npos) {
        ...
        if (ops[cat] != 0)
          EXCEPT_PARAM(
              1, "Invalid query - categories may only be specified once.");
