...
#define TRANSLATE_END(json)                                \
  }                                                        \
  catch (TraceException & e) {                             \
    DL_ERROR("INVALID JSON FROM DB: " << json.toString()); \
...
DatabaseAPI::DatabaseAPI(const std::string& a_db_url,
  ...
  if (!m_curl) EXCEPT(ID_INTERNAL_ERROR, "libcurl init failed");
...
DatabaseAPI::~DatabaseAPI() {
  if (m_client) curl_free(m_client);
...
long DatabaseAPI::dbGet(const char* a_url_path,
                        ...
                        libjson::Value& a_result, bool a_log) {
  (void)a_log;
...
bool DatabaseAPI::userGetKeys(std::string& a_pub_key, std::string& a_priv_key) {
  ...
  if (!obj.has("pub_key")) return false;

  a_pub_key = obj.asString();

  if (!obj.has("priv_key")) return false;
...
void DatabaseAPI::userClearKeys() {
  Value result;
...
void DatabaseAPI::userSetAccessToken(const std::string& a_acc_tok,
  ...
  // DL_DEBUG("userSetAccessToken: " << a_acc_tok  << " : " << a_ref_tok << " :
  // " << a_expires_in );
  string result;
...
void DatabaseAPI::userCreate(const Auth::UserCreateRequest& a_request,
  ...
  DL_INFO("DataFed user create - uid: " << a_request.uid()
                                        << ", name: " << a_request.name());
...
void DatabaseAPI::userUpdate(const UserUpdateRequest& a_request,
  ...
  vector<pair<string, string>> params;
  params.push_back({"subject", a_request.uid()});
  if (a_request.has_email()) params.push_back({"email", a_request.email()});
  if (a_request.has_password())
    params.push_back({"password", a_request.password()});
  if (a_request.has_options())
    params.push_back({"options", a_request.options()});
...
void DatabaseAPI::setUserData(Auth::UserDataReply& a_reply,
  ...
  for (Value::ArrayConstIter i = arr.begin(); i != arr.end(); i++) {
    ...
    if (obj.has("paging")) {
      ...
      if (obj.has("email")) user->set_email(obj.asString());

      if (obj.has("options")) user->set_options(obj.asString());

      if (obj.has("is_admin")) user->set_is_admin(obj.asBool());

      if (obj.has("is_repo_admin")) user->set_is_repo_admin(obj.asBool());
...
void DatabaseAPI::projCreate(const Auth::ProjectCreateRequest& a_request,
  ...
  if (a_request.has_desc()) params.push_back({"desc", a_request.desc()});
...
void DatabaseAPI::projUpdate(const Auth::ProjectUpdateRequest& a_request,
  ...
  if (a_request.has_title()) params.push_back({"title", a_request.title()});
...
void DatabaseAPI::setProjectData(Auth::ProjectDataReply& a_reply,
  ...
  for (Value::ArrayConstIter i = arr.begin(); i != arr.end(); i++) {
    ...
    if (obj.has("desc")) proj->set_desc(obj.asString());

    if (obj.has("owner")) proj->set_owner(obj.asString());

    if (obj.has("ct")) proj->set_ct(obj.asNumber());

    if (obj.has("ut")) proj->set_ut(obj.asNumber());
...
void DatabaseAPI::recordCreate(const Auth::RecordCreateRequest& a_request,
  ...
  if (a_request.has_alias()) body += ",\"alias\":\"" + a_request.alias() + "\"";
  ...
  if (a_request.has_metadata()) body += ",\"md\":" + a_request.metadata();
  ...
  if (a_request.has_external()) {
    ...
    if (a_request.has_ext()) body += ",\"ext\":\"" + a_request.ext() + "\"";
    if (a_request.has_ext_auto())
      body +=
          string(",\"ext_auto\":") + (a_request.ext_auto() ? "true" : "false");
  }
  ...
  if (a_request.deps_size()) {
    ...
    for (int i = 0; i < a_request.deps_size(); i++) {
      body += string(i > 0 ? "," : "") + "{\"id\":\"" + a_request.deps(i).id() +
              "\",\"type\":" + to_string(a_request.deps(i).type()) + "}";
    }
    body += "]";
  }
  body += "}";
...
void DatabaseAPI::recordUpdate(const Auth::RecordUpdateRequest& a_request,
  ...
  if (a_request.has_metadata()) {
    body += ",\"md\":" +
            (a_request.metadata().size() ? a_request.metadata() : "\"\"");
  ...
  if (a_request.dep_add_size()) {
    ...
    for (int i = 0; i < a_request.dep_add_size(); i++) {
      body += string(i > 0 ? "," : "") + "{\"id\":\"" +
              a_request.dep_add(i).id() +
              "\",\"type\":" + to_string(a_request.dep_add(i).type()) + "}";
    }
    body += "]";
  }
  ...
  if (a_request.dep_rem_size()) {
    ...
    for (int i = 0; i < a_request.dep_rem_size(); i++) {
      body += string(i > 0 ? "," : "") + "{\"id\":\"" +
              a_request.dep_rem(i).id() +
              "\",\"type\":" + to_string(a_request.dep_rem(i).type()) + "}";
    }
    body += "]";
  }
...
void DatabaseAPI::recordUpdateSize(const Auth::RepoDataSizeReply& a_size_rep) {
  ...
  for (int i = 0; i < a_size_rep.size_size(); i++) {
    if (i > 0) body += ",";
    body += "{\"id\":\"" + a_size_rep.size(i).id() +
            "\",\"size\":" + to_string(a_size_rep.size(i).size()) + "}";
  }
...
void DatabaseAPI::setRecordData(Auth::RecordDataReply& a_reply,
  ...
  if (res_obj.has("results")) {
    ...
    for (i = arr.begin(); i != arr.end(); i++) {
      ...
      if (obj.has("owner")) rec->set_owner(obj.asString());

      if (obj.has("creator")) rec->set_creator(obj.asString());

      if (obj.has("desc")) rec->set_desc(obj.asString());
      ...
      if (obj.has("md")) rec->set_metadata(obj.value().toString());

      if (obj.has("md_err_msg")) rec->set_md_err_msg(obj.asString());

      if (obj.has("sch_id")) rec->set_sch_id(obj.asString());

      if (obj.has("external")) rec->set_external(obj.asBool());

      if (obj.has("repo_id")) rec->set_repo_id(obj.asString());

      if (obj.has("size")) rec->set_size(obj.asNumber());

      if (obj.has("source")) rec->set_source(obj.asString());

      if (obj.has("ext")) rec->set_ext(obj.asString());

      if (obj.has("ext_auto")) rec->set_ext_auto(obj.asBool());

      if (obj.has("ct")) rec->set_ct(obj.asNumber());

      if (obj.has("ut")) rec->set_ut(obj.asNumber());

      if (obj.has("dt")) rec->set_dt(obj.asNumber());

      if (obj.has("locked")) rec->set_locked(obj.asBool());

      if (obj.has("parent_id")) rec->set_parent_id(obj.asString());

      if (obj.has("notes")) rec->set_notes(obj.asNumber());
...
void DatabaseAPI::generalSearch(const Auth::SearchRequest& a_request,
  ...
  uint32_t cnt =
      parseSearchRequest(a_request, qry_begin, qry_end, qry_filter, params);
...
void DatabaseAPI::collMove(const Auth::CollMoveRequest& a_request,
  ...
  if (a_request.item_size() == 0) return;
...
void DatabaseAPI::setCollData(Auth::CollDataReply& a_reply,
  ...
  if (res_obj.has("results")) {
    ...
    for (i = arr.begin(); i != arr.end(); i++) {
      ...
      if (obj.has("desc")) coll->set_desc(obj.asString());

      if (obj.has("topic")) coll->set_topic(obj.asString());
      ...
      if (obj.has("ct")) coll->set_ct(obj.asNumber());

      if (obj.has("ut")) coll->set_ut(obj.asNumber());

      if (obj.has("parent_id")) coll->set_parent_id(obj.asString());

      if (obj.has("owner")) coll->set_owner(obj.asString());

      if (obj.has("creator")) coll->set_creator(obj.asString());

      if (obj.has("notes")) coll->set_notes(obj.asNumber());
    }
  }
...
void DatabaseAPI::setCollPathData(CollPathReply& a_reply,
  ...
  for (Value::ArrayConstIter i = arr.begin(); i != arr.end(); i++) {
    ...
    for (j = arr2.begin(); j != arr2.end(); j++) {
      ...
      if (obj.has("owner")) item->set_owner(obj.asString());
    }
  }
...
void DatabaseAPI::setListingData(ListingData* a_item,
  ...
  if (a_obj.has("notes")) a_item->set_notes(a_obj.asNumber());

  if (a_obj.has("locked") && !a_obj.value().isNull())
    a_item->set_locked(a_obj.asBool());

  if (a_obj.has("gen")) a_item->set_gen(a_obj.asNumber());
  ...
  if (a_obj.has("deps")) {
    ...
    for (Value::ArrayConstIter i = arr2.begin(); i != arr2.end(); i++) {
      ...
      if (obj2.has("notes")) dep->set_notes(obj2.asNumber());
    }
  }
}
...
void DatabaseAPI::aclUpdate(const Auth::ACLUpdateRequest& a_request,
  ...
  vector<pair<string, string>> params;
  params.push_back({"id", a_request.id()});
  if (a_request.has_rules()) params.push_back({"rules", a_request.rules()});
...
void DatabaseAPI::setACLData(ACLDataReply& a_reply,
  ...
  for (Value::ArrayConstIter i = arr.begin(); i != arr.end(); i++) {
    ...
    if (obj.has("grant")) rule->set_grant(obj.asNumber());

    if (obj.has("inhgrant")) rule->set_inhgrant(obj.asNumber());
  }
...
void DatabaseAPI::setGroupData(GroupDataReply& a_reply,
  ...
  for (Value::ArrayConstIter i = arr.begin(); i != arr.end(); i++) {
    ...
    if (obj.has("uid") && !obj.value().isNull()) group->set_uid(obj.asString());
...
void DatabaseAPI::setRepoData(Auth::RepoDataReply* a_reply,
  ...
  for (Value::ArrayConstIter i = arr.begin(); i != arr.end(); i++) {
    ...
    if (obj.has("capacity")) {
      a_repos.back().set_capacity(obj.asNumber());  // TODO Needs to be 64 bit
                                                    // integer (string in JSON)
    }

    if (obj.has("address")) {
      std::cout << "Setting address for repo " << obj.getString("id")
                << " address is " << obj.asString() << std::endl;
...
void DatabaseAPI::setAllocData(AllocData* a_alloc,
  ...
  if (a_obj.has("is_def")) a_alloc->set_is_def(a_obj.asBool());

  if (a_obj.has("id")) a_alloc->set_id(a_obj.asString());

  if (a_obj.has("stats")) setAllocStatsData(*a_alloc->mutable_stats(), a_obj);
}
...
void DatabaseAPI::topicListTopics(const Auth::TopicListTopicsRequest& a_request,
  ...
  Value result;
  vector<pair<string, string>> params;
  if (a_request.has_topic_id()) params.push_back({"id", a_request.topic_id()});
  if (a_request.has_offset() && a_request.has_count()) {
    params.push_back({"offset", to_string(a_request.offset())});
    params.push_back({"count", to_string(a_request.count())});
  }
...
void DatabaseAPI::setTopicDataReply(Auth::TopicDataReply& a_reply,
  ...
  for (Value::ArrayConstIter i = arr.begin(); i != arr.end(); i++) {
    ...
    if (obj.has("paging")) {
      ...
      if (obj.has("desc")) topic->set_desc(obj.asString());

      if (obj.has("creator")) topic->set_creator(obj.asString());
...
void DatabaseAPI::notePurge(uint32_t a_age_sec) {
  Value result;
...
void DatabaseAPI::setNoteData(NoteData* a_note,
  ...
  if (a_obj.has("has_child")) a_note->set_has_child(a_obj.asBool());
...
void DatabaseAPI::schemaSearch(const Auth::SchemaSearchRequest& a_request,
  ...
  if (a_request.has_id()) params.push_back({"id", a_request.id()});
  if (a_request.has_text()) params.push_back({"text", a_request.text()});
  if (a_request.has_owner()) params.push_back({"owner", a_request.owner()});
  if (a_request.has_sort())
    params.push_back({"sort", to_string(a_request.sort())});
...
void DatabaseAPI::setSchemaData(SchemaData* a_schema,
  ...
  if (a_obj.has("cnt")) a_schema->set_cnt(a_obj.asNumber());
  ...
  if (a_obj.has("desc")) a_schema->set_desc(a_obj.asString());

  if (a_obj.has("pub")) a_schema->set_pub(a_obj.asBool());

  if (a_obj.has("depr")) a_schema->set_depr(a_obj.asBool());

  if (a_obj.has("ref")) a_schema->set_ref(a_obj.asBool());

  if (a_obj.has("def")) a_schema->set_def(a_obj.value().toString());
...
void DatabaseAPI::schemaView(const std::string& a_id,
...
void DatabaseAPI::taskRun(const std::string& a_task_id,
                          libjson::Value& a_task_reply, int* a_step,
...
void DatabaseAPI::taskInitDataGet(const Auth::DataGetRequest& a_request,
  ...
  if (a_request.has_path()) body += ",\"path\":\"" + a_request.path() + "\"";
  ...
  if (a_request.has_check() && a_request.check()) body += ",\"check\":true";
...
void DatabaseAPI::setDataGetReply(Auth::DataGetReply& a_reply,
  ...
  if (obj.has("task")) setTaskData(a_reply.mutable_task(), obj.value());
...
void DatabaseAPI::setDataPutReply(Auth::DataPutReply& a_reply,
  ...
  if (rec.has("size") && !rec.value().isNull()) item->set_size(rec.asNumber());
...
void DatabaseAPI::setTaskDataReply(Auth::TaskDataReply& a_reply,
  ...
  if (obj.has("task")) setTaskData(a_reply.add_task(), obj.value());
...
void DatabaseAPI::taskUpdate(const std::string& a_id, TaskStatus* a_status,
                             const std::string* a_message, double* a_progress,
                             libjson::Value* a_state) {
  if (!a_status && !a_progress && !a_state) return;
...
void DatabaseAPI::taskFinalize(const std::string& a_task_id, bool a_succeeded,
  ...
  if (a_msg.size()) params.push_back({"message", a_msg});
...
void DatabaseAPI::taskList(const Auth::TaskListRequest& a_request,
  ...
  if (a_request.has_to()) params.push_back({"to", to_string(a_request.to())});
  if (a_request.has_offset())
    params.push_back({"offset", to_string(a_request.offset())});
  if (a_request.has_count())
    params.push_back({"count", to_string(a_request.count())});
...
void DatabaseAPI::taskPurge(uint32_t a_age_sec) {
  libjson::Value result;
...
void DatabaseAPI::metricsPurge(uint32_t a_timestamp) {
  libjson::Value result;
...
uint32_t DatabaseAPI::parseSearchRequest(const Auth::SearchRequest& a_request,
  ...
  a_qry_end +=
      " let name = (for j in u filter j._id == i.owner return "
  ...
  a_qry_end +=
      string(
          " return distinct "
          "{_id:i._id,title:i.title,'desc':i['desc'],owner:i.owner,owner_name:"
          "name,alias:i.alias") +
...
string DatabaseAPI::parseSearchTextPhrase(const string& a_phrase,
  ...
  */
  ...
  // Build OR phrase
  if (count_or > 1 && count_other > 0) result += "(";

  if (ops[1] == 1) result += parseSearchTerms("title", title, a_iter);
...
std::string DatabaseAPI::parseSearchTerms(
    const std::string& a_key, const std::vector<std::string>& a_terms,
  ...
  if (or_terms.size() > 1) result += "(";

  for (i = or_terms.begin(); i != or_terms.end(); ++i) {
    if (i != or_terms.begin()) result += " or ";
  ...
  for (string::const_iterator c = a_query.begin(); c != a_query.end(); c++) {
    ...
    switch (state) {
      case PS_SINGLE_QUOTE:  // Single quote (not escaped)
        ...
        // backslashes (\\) For example: 'abc\'' - quote is escaped 'abc\\'' -
        // backslash is escaped, not quote 'abc\\\''  - backslash and quote are
        // escaped 'abc\\\\''  - two backslashes are escaped, and quote The rule
        // is: if the number of preceeding contiguous backslashes is even, then
        // the quote is NOT escaped This applies to double quotes in the
        // subsequent case as well
      ...
      case PS_DEFAULT:   // Not quoted, not an identifier
        ...
        } else if (defchar.find(*c) != defchar.end())  // Check for other
                                                       // allowed characters
        {
                                                     ...
                                                     // "/"
  {
    if (p == 0 || (p == 1 && val[0] == 'd')) {
      ...
      if (val.size() >= p + 3) {
        ...
        if (id_ok) return a_iter + "._id like 'd/" + val.substr(p + 1) + "%'";
      }
    }
