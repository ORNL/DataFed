...
TaskMgr::TaskMgr()
  ...
  Currently the only moderately safe way for libraries using OpenSSL to handle
  thread safety is to do the following as early as possible, possible in .init
  or DllMain:
  ...
  In the future we hope that OpenSSL will self-initialize thread-safely to use
  native threading where available.
  */
  /*
  ...
  */
  ...
  DL_DEBUG("TaskMgr creating " << m_config.num_task_worker_threads
                               << " task worker threads.");
 ...
 */
void TaskMgr::maintenanceThread() {
  ...
  while (1) {
    ...
    timeout = purge_next;
    DL_INFO("MAINT: Next purge: " << chrono::duration_cast<chrono::seconds>(
    ...
    if (t != m_tasks_retry.end()) {
      ...
      if (t->first < purge_next) {
        timeout = t->first;
        DL_INFO("MAINT: timeout based on next retry: "
    ...
    DL_INFO("MAINT: timeout: " << chrono::duration_cast<chrono::seconds>(
...
void TaskMgr::purgeTaskHistory() const {
  try {
    DatabaseAPI db(m_config.db_url, m_config.db_user, m_config.db_pass);
 ...
 */
void TaskMgr::newTask(const std::string &a_task_id) {
 ...
 */
void TaskMgr::newTasks(const libjson::Value &a_tasks) {
  try {
...
void TaskMgr::retryTaskAndScheduleWorker(Task *a_task) {
...
void TaskMgr::cancelTask(const std::string &a_task_id) {
 ...
 */
TaskMgr::Task *TaskMgr::getNextTask(ITaskWorker *a_worker) {
  Task *task = 0;
 ...
 */
bool TaskMgr::retryTask(Task *a_task) {
  ...
  if (a_task->retry_count == 0) {
    ...
    DL_DEBUG("Retry time " << chrono::duration_cast<chrono::seconds>(
                                  a_task->retry_time.time_since_epoch())
                                  .count());
    DL_DEBUG("Fail time " << chrono::duration_cast<chrono::seconds>(
    ...
    DL_DEBUG("Next retry time " << chrono::duration_cast<chrono::seconds>(
                                       a_task->retry_time.time_since_epoch())
                                       .count());
