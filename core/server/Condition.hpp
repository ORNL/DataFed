...
class Promote : public Condition {
  ...
  public:
    ...
    Promote(const size_t access_attempts,
        const PublicKeyType promote_from,
        const PublicKeyType promote_to) :
      m_transient_to_session_count_threshold(access_attempts),
      m_promote_from(promote_from),
      m_promote_to(promote_to) {};

    virtual ConditionType type() const noexcept final { return ConditionType::PROMOTION_TO_SESSION_AND_PURGE_FROM_TRANSIENT; }
...
class Reset : public Condition {
  ...
  public:
    ...
    Reset(
        const size_t access_attempts,
        const PublicKeyType key_type) : 
      m_access_attempts(access_attempts),
      m_act_on_key_type(key_type) {};

    virtual ConditionType type() const noexcept final { return ConditionType::RESET_IF_ACCESSED_ELSE_PURGE; }
