 ...
 */
void Server::run() {
  DL_INFO("Public/private MAPI starting on ports " << m_config.port << "/"
                                                   << (m_config.port + 1))
...
void Server::ioInsecure() {
  try {
    ...
    //        MsgComm frontend( "tcp://*:" + to_string(m_config.port + 1),
    //        MsgComm::ROUTER, true );
...
void Server::metricsThread() {
  ...
  DL_DEBUG("metrics: " << m_config.metrics_purge_period << ", "
                       << m_config.metrics_period << ", " << purge_count);
...
Server::zapHandler()
{
    ...
    try
    {
        ...
        char        mechanism[100];
        char        client_key[100];
        zmq_pollitem_t                  poll_items[] = { socket, 0, ZMQ_POLLIN,
0 };
...
) EXCEPT( 1, "Rcv identity_property failed." ); identity_property[rc] = 0; if ((
rc = zmq_recv( socket, mechanism, 100, 0 )) == -1 ) EXCEPT( 1, "Rcv mechanism
failed." ); mechanism[rc] = 0; if (( rc = zmq_recv( socket, client_key, 100, 0
)) == -1 ) EXCEPT( 1, "Rcv client_key failed." ); client_key[rc] = 0;
                ...
                zmq_send( socket, request_id, strlen(request_id), ZMQ_SNDMORE );
                                                                // A value of
200 means accept a value of 400 means deny zmq_send( socket, "200", 3,
ZMQ_SNDMORE ); zmq_send( socket, "", 0, ZMQ_SNDMORE ); zmq_send( socket,
client_key_text, strlen(client_key_text), ZMQ_SNDMORE ); zmq_send( socket, "",
0, 0 );
...
void Server::authenticateClient(const std::string &a_cert_uid,
  ...
  DL_INFO("authenticateClient a_cert_uid is " << a_cert_uid << " a_uid is "
                                              << a_uid);
