...
class Server : public ICoreServer {
 ...
 private:
  ...
  bool isClientAuthenticated(const std::string& a_client_key,
                             std::string& a_uid);
  ...
  // std::thread                   m_zap_thread;           ///< ZeroMQ client
  // authentication (ZAP) thread
  std::thread m_msg_router_thread;  ///< Main message router thread handle
  std::vector<std::shared_ptr<ClientWorker>> m_workers;  ///< List of
                                                         ///< ClientWorker
                                                         ///< instances
  ...
  std::map<std::string, MsgMetrics_t> m_msg_metrics;  ///< Map of UID to message
                                                      ///< request metrics
