...
class AuthMap {
 ...
 private:
  ...
  mutable std::mutex m_trans_clients_mtx;  ///< Mutex for transient client data
                                           ///< access
  mutable std::mutex m_session_clients_mtx;  ///< Mutex for session client data
                                             ///< access
  mutable std::mutex m_persistent_clients_mtx;  ///< Mutex for persistent client
                                                ///< data access

  client_map_t m_trans_auth_clients;    ///< Map of transient authenticated
                                        ///< clients
  client_map_t m_session_auth_clients;  ///< Map of session authenticated
                                        ///< clients
  persistent_client_map_t m_persistent_auth_clients;  ///< Map of known
                                                      ///< persistent
 ...
 public:
  ...
  std::vector<std::string> getExpiredKeys(
      const PublicKeyType pub_key_type, const time_t threshold) const noexcept;
