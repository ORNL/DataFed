...
GlobusAPI::GlobusAPI() : m_config(Config::getInstance()) {
  ...
  if (!m_curl_xfr) EXCEPT(1, "libcurl init failed");
  ...
  if (!m_curl_auth) EXCEPT(1, "libcurl init failed");
...
long GlobusAPI::get(CURL *a_curl, const std::string &a_base_url,
  ...
  if (res != CURLE_OK) {
    DL_ERROR("GlobusAPI::get - CURL error [" << error << "], "
...
long GlobusAPI::post(
  ...
  if (res != CURLE_OK) {
    DL_ERROR("GlobusAPI::post - CURL error [" << error << "], "
                                              << curl_easy_strerror(res));
    EXCEPT_PARAM(1, "Globus API call failed.");
  }
  ...
  try {
    ...
    if (status == "ACTIVE") {
      ...
      if (faults > 0.0) {
        ...
        std::cout << "faults encountered for task: " << a_task_id << " faults "
                  << faults << std::endl;
        string raw_result2;
