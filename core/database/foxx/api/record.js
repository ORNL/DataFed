...
class Record  {
  ...
  constructor(a_key) {
    ...
    const collection = db._collection('d');
    ...
    if (a_key) {
      ...
      this.exists = collection.exists(key);
      this.key = a_key;
    }
    this.loc = null;
    this.alloc = null;
  ...
  isPathConsistent(a_path) {
    ...
    if (this.alloc.path + this.key !== a_path) {

      // This may be due to an alloc/owner change allow if a new path matches
      // If it is not a new repo throw an error because we have
      // already determine that it is not part of the old repository
      // allocation. The path should only be different if it is a new
      // repo.
      if (!this.loc.new_repo) {
        this.error = g_lib.ERR_PERM_DENIED;
        this.err_msg = "Record path is not consistent with repo expected path is: " + this.alloc.path + this.key + " attempted path is " + a_path;
        return false;
      }
      ...
      if (new_alloc.path + this.key !== a_path) {
        this.error = g_lib.ERR_PERM_DENIED;
        this.err_msg = "Permission denied, DataFed registered path is '" + new_alloc.path + this.key + "' Globus path is '" + a_path + "'"
        return false;
      }
    }
    return true;
