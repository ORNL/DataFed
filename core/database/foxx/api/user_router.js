...
router.get('/authn/token', function(req, res) {
        try {
            var user = g_db._query("for i in u filter i.access == @tok return i", {
        ...
        try {
            ...
            g_db._executeTransaction({
                ...
                action: function() {
                    ...
                    var user = g_db.u.save(user_data, {
                        returnNew: true
                    });
                    var root = g_db.c.save({
                        ...
                        desc: "Root collection for user " + req.queryParams.name + " (" + req.queryParams.uid + ")",
                        alias: "root"
                    }, {
                        returnNew: true
                    });
...
router.get('/find/by_uuids', function(req, res) {
        try {
            ...
            var idents = g_db._query("for v in 1..1 outbound @user ident return v._key", {
                user: user._id
            }).toArray();
            if (idents.length) {
                user.idents = idents;
...
router.get('/token/get/expiring', function(req, res) {
        try {
            ...
            var results = g_db._query("for i in u filter i.expiration != Null && i.expiration < @exp return {id:i._id,access:i.access,refresh:i.refresh,expiration:i.expiration}", {
...
router.get('/view', function(req, res) {
        try {
            ...
            if (det_ok) {
                var repos = g_db._query("for v in 1..1 inbound @user admin filter is_same_collection('repo',v) limit 1 return v._key", {
                    user: user._id
                }).toArray();
                if (repos.length)
                    user.is_repo_admin = true;

                user.allocs = g_db.alloc.byExample({
                    _from: user._id
                }).toArray();
        ...
        try {
            g_db._executeTransaction({
                ...
                action: function() {
                    ...
                    objects = g_db._query("for v in 1..1 outbound @user ident return v._id", {
                        user: user_id
                    }).toArray();
                    for (i in objects) {
                        obj = objects[i];
                    ...
                    objects = g_db._query("for v in 1..1 inbound @user owner filter is_same_collection('p',v) return v._id", {
                        user: user_id
                    }).toArray();
                    for (i in objects) {
                        obj = objects[i];
                        //console.log( "del proj", obj );
                        subobjects = g_db._query("for v in 1..1 inbound @proj owner return v._id", {
                            proj: obj
                        }).toArray();
                        for (j in subobjects) {
                            subobj = subobjects[j];
                    ...
                    objects = g_db._query("for v in 1..1 inbound @user owner return v._id", {
                        user: user_id
                    }).toArray();
                    for (i in objects) {
                        obj = objects[i];
