...
router.get('/list', function(req, res) {
        ...
        if (!client) {
            ...
            if (req.queryParams.details) {
                result = g_db._query("for v in 1..1 inbound @admin admin filter is_same_collection('repo',v) return v", {
                    admin: client._id
                }).toArray();
                for (i in result) {
                    repo = result[i];
                ...
                result = g_db._query("for v in 1..1 inbound @admin admin filter is_same_collection('repo',v) return {id:v._id,title:v.title,domain:v.domain}", {
                    admin: client._id
                });
            }
        }
...
router.get('/view', function(req, res) {
        try {
            ...
            repo.admins = [];
            var admins = g_db.admin.byExample({
            ...
            g_db._executeTransaction({
                ...
                action: function() {
                  ...
                  FOR edge IN lock
                    ...
                    RETURN edge._from
                `
                    var items_connected_to_repo = g_db._query(aqlQuery, {
...
router.get('/alloc/list/by_repo', function(req, res) {
        ...
        var result = g_db._query("for v, e in 1..1 inbound @repo alloc sort v._id return {id:v._id,repo:@repo,data_limit:e.data_limit,data_size:e.data_size,rec_limit:e.rec_limit,rec_count:e.rec_count,path:e.path}", {
            repo: repo._id
        }).toArray();
...
router.get('/alloc/list/by_owner', function(req, res) {
        var obj, result = g_db.alloc.byExample({
...
function getAllocStats(a_repo, a_subject) {
    ...
    if (a_subject) {
        ...
        sizes = g_db._query("for v,e,p in 1..1 inbound @repo loc filter e.uid == @subj return v.size", {
            repo: a_repo,
            subj: a_subject
        });
    } else {
        ...
        try {
            g_db._executeTransaction({
                ...
                action: function() {
                    ...
                    var alloc, allocs = g_db.alloc.byExample({
                        _from: subject_id
                    });
