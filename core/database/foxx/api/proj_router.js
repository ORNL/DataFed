            ...
            g_db._executeTransaction({
                ...
                action: function() {
                    ...
                    if (client.max_proj >= 0) {
                        var count = g_db._query("return length(FOR i IN owner FILTER i._to == @id and is_same_collection('p',i._from) RETURN 1)", {
                            id: client._id
                        }).next();
                ...
                action: function() {
                    ...
                    if (req.queryParams.admins) {
                        ...
                        for (i in req.queryParams.admins) {
                            ...
                            links = g_db._query("for v,e,p in 2..2 inbound @user acl, outbound owner filter v._id == @proj return p.edges[0]._id", {
                                user: uid,
                                proj: proj_id
                            });
                            while (links.hasNext()) {
                            ...
                            links = g_db._query("for v,e,p in 2..2 inbound @user member, outbound owner filter v._id == @proj return p.edges[0]._id", {
                                user: uid,
                                proj: proj_id
                            });
                            while (links.hasNext()) {
                        ...
                        var admins = g_db._query("for i in admin filter i._from == @proj return i._to", {
                            proj: proj_id
                        }).toArray();
                        for (i in admins) {
                    ...
                    if (req.queryParams.members) {
                        ...
                        var members = g_db._query("for v,e,p in 2..2 inbound @proj owner, outbound member filter p.vertices[1].gid == 'members' return v._id", {
                            proj: proj_id
                        }).toArray();
...
router.get('/view', function(req, res) {
        try {
            ...
            var admins = g_db._query("for v in 1..1 outbound @proj admin return v._id", {
                proj: proj._id
            }).toArray();
            if (admins.length) {
                proj.admins = admins;
            ...
            if (client) {
                ...
                proj.allocs = g_db.alloc.byExample({
                    _from: proj._id
                }).toArray();
