...
describe("integration_repo_router: the Foxx microservice repo_router create endpoint", () => {
    ...
    it("should view a repo by id", () => {
        ...
        // act
        const response = request.get(`${repo_base_url}/view?id=repo/heavymetal`);
        ...
        expect(repo.admins).to.have.members(["u/shredder", "u/splinter"]);
    ...
    it("should create an allocation for a user/project when repo admin", () => {
        ...
        const requestString =
            `${repo_base_url}/alloc/create?client=${clientId}` +
    ...
    it("should set default allocation for self", () => {
        ...
        const request_string = `${repo_base_url}/alloc/set/default?client=alloc_user&repo=repo/repo1`;
