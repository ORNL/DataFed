...
describe("unit_data_router: the Foxx microservice data_router create/ endpoint", () => {
    beforeEach(() => {
        ...
        collections.forEach((name) => {
            let col = db._collection(name);
            if (col)
                col.truncate(); // truncate if exists
    ...
    it("should update multiple data records in a batch", () => {
        ...
        const createRes = request.post(`${data_base_url}/create/batch?client=fakeUser`, {
            body: [{ title: "Batch Original 1" }, { title: "Batch Original 2" }],
            json: true,
        });
        ...
        const createBody =
            typeof createRes.body === "string" ? JSON.parse(createRes.body) : createRes.body;
        ...
        const updateBody =
            typeof updateRes.body === "string" ? JSON.parse(updateRes.body) : updateRes.body;
    ...
    it("should export one or more data records as JSON strings via /export", () => {
        ...
        const createRes = request.post(`${data_base_url}/create/batch?client=fakeUser`, {
            body: [{ title: "Export Record 1" }, { title: "Export Record 2" }],
            json: true,
        });
        expect(createRes.statusCode).to.equal(200);
        ...
        const exportBody =
            typeof exportRes.body === "string" ? JSON.parse(exportRes.body) : exportRes.body;
    ...
    it("should return a dependency graph for a data record", () => {
        ...
        const [rootId, childId, grandchildId] = createBody.results.map((r) => r.id);
    ...
    it("should lock multiple data records", () => {
        ...
        const lockBody = typeof lockRes.body === "string" ? JSON.parse(lockRes.body) : lockRes.body;
    ...
    it("should list data records by allocation for a repo", () => {
        // Step 1: create two data records
        const createRes = request.post(`${data_base_url}/create/batch?client=fakeUser`, {
            body: [{ title: "Alloc Record One" }, { title: "Alloc Record Two" }],
            json: true,
        });
        ...
        const res = request.get(`${data_base_url}/list/by_alloc?client=fakeUser&repo=${repoId}`);
    ...
    it("should initialize an allocation change task for data records", () => {
        // Step 1: create two data records
        const createRes = request.post(`${data_base_url}/create/batch?client=fakeUser`, {
            body: [{ title: "AllocChg Record 1" }, { title: "AllocChg Record 2" }],
            json: true,
        });
    ...
    it("should initialize a delete task for data records", () => {
        ...
        const createRes = request.post(`${data_base_url}/create/batch?client=fakeUser`, {
            body: [{ title: "Delete Test Record 1" }, { title: "Delete Test Record 2" }],
            json: true,
        });
