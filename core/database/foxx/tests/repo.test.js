...
describe("Testing Repo class", () => {
 ...
 it("unit_repo: testing getProjectIds for a repo with multiple projects", () => {
  ...
  it("unit_repo: testing getProjectIds for invalid allocations (not projects)", () => {
    ...
    const repo_id = "repo/" + repo_key;
    g_db.repo.save({ _id: repo_id, _key: repo_key, path: "/mnt/invalid" }, { waitForSync: true });
  ...
  it("unit_repo: testing getProjectIds for multiple repos with overlapping projects", () => {
      ...
      g_db.repo.save({ _id: repo1_id, _key: repo1_key, path: "/mnt/repo1" }, { waitForSync: true });
      g_db.repo.save({ _id: repo2_id, _key: repo2_key, path: "/mnt/repo2" }, { waitForSync: true });
  ...
  it("unit_repo: testing hasAccess for a user with a direct allocation", () => {
      ...
      const repo_id = "repo/" + repo_key;
      g_db.repo.save({ _id: repo_id, _key: repo_key, path: "/mnt/direct_alloc" }, { waitForSync: true });
  ...
  it("unit_repo: testing hasAccess for a user with no allocation", () => {
      ...
      const repo_id = "repo/" + repo_key;
      g_db.repo.save({ _id: repo_id, _key: repo_key, path: "/mnt/no_alloc" }, { waitForSync: true });
   ...
   it("unit_repo: testing hasAccess for a user with allocation to a different repo", () => {
    ...
    g_db.repo.save({ _id: repo_id_1, _key: repo_key_1, path: "/mnt/other" }, { waitForSync: true });
    g_db.repo.save({ _id: repo_id_2, _key: repo_key_2, path: "/mnt/different" }, { waitForSync: true });
