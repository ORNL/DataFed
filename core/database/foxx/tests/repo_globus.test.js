...
describe("unit_repository_globus: Globus Repository Operations", function () {
    ...
    beforeEach(function () {
        const collections = ["d", "block", "alloc", "loc", "lock", "repo", "admin", "task", "g", "p", "u"];
   ...
   function getValidRawAllocationData() {
    ...
    describe("unit_repository_globus: createAllocation", function () {
        it("unit_repository_globus: should fail to create allocation without admin role", function () {
            ...
            expect(result.error.message).to.include("Client, 'u/456', does not have administrative repository permissions on repo/123");
        });
        ...
        it("unit_repository_globus: should fail to create allocation if a duplicate task is found", function () {
            ...
            expect(result.ok).to.be.false;
            expect(result.error.message).to.include("A duplicate allocation create task was found: task/43")
        });
  ...
  describe("unit_repository_globus: deleteAllocation", function () {
    it("unit_repository_globus: should delete allocation with valid subject ID", function () {
      ...
      const result = globus_repo.deleteAllocation({_id: userRawData._id, is_admin: false }, userRawData._id);
    ...
    it("unit_repository_globus: should delete allocation with valid system level admin permissions.", function () {
      ...
      const result = globus_repo.deleteAllocation({_id: userRawData._id, is_admin: true }, userRawData._id);
    ...
    it("unit_repository_globus: should reject deletion with null subject ID", function () {
      ...
      const result = globus_repo.deleteAllocation({_id: userRawData._id, is_admin: false }, null);

      expect(result.ok).to.be.false;

      expect(result.error.message).to.include("Subject ID is required for allocation deletion");
    });
    ...
    it("unit_repository_globus: should reject deletion if client does not have permissions", function () {
      ...
      expect(result.ok).to.be.false;
      expect(result.error.message).to.include("Failed to create allocation task: Client, 'u/456', does not have administrative repository permissions on repo/123");
    });
    ...
    it("unit_repository_globus: should reject allocation deletion when allocation does not exist", function () {
      ...
      expect(result.ok).to.be.false;
      expect(result.error.message).to.include("Failed to create allocation task: Subject, \'u/456\', has no allocation on repo/123");
    });
    ...
    it("unit_repository_globus: should reject duplicate allocation deletion.", function () {
      ...
      globus_repo.deleteAllocation({_id: userRawData._id, is_admin: false }, userRawData._id);
      const result = globus_repo.deleteAllocation({_id: userRawData._id, is_admin: false }, userRawData._id);

      expect(result.ok).to.be.false;
      expect(result.error.message).to.include("Failed to create allocation task: A duplicate allocation delete task was found: task/");
    });
