...
describe("unit_globus_token_model: Calling constructor", () => {
    it("should succeed when provided a user ID and Globus collection ID", () => {
        const globus_token_model = new GlobusTokenModel(get_globus_token_model_user_id, get_globus_token_model_collection_id);
        expect(globus_token_model).to.be.instanceOf(GlobusTokenModel);
    });
    it("should throw an error when a user ID is not provided", () => {
        expect(()  => new GlobusTokenModel(null, get_globus_token_model_collection_id)).to.throw("User ID and Collection ID are required for Globus Tokens");
    });
    it("should throw an error when a collection ID is not provided", () => {
        expect(()  => new GlobusTokenModel(get_globus_token_model_user_id, null)).to.throw("User ID and Collection ID are required for Globus Tokens");
    });
    it("should throw an error when neither a user ID nor a collection ID are provided", () => {
        expect(()  => new GlobusTokenModel(null, null)).to.throw("User ID and Collection ID are required for Globus Tokens");
    });
});
...
describe("unit_globus_token_model: Calling get", () => {
    it("should return a read only globus token object when the globus token exists", () => {
        const globus_token = new GlobusTokenModel(get_globus_token_model_user_id, get_globus_token_model_collection_id).get();
...
describe("unit_globus_token_model: Calling get_oauth_token", () => {
    it("should return a read only DataFed OAuth token object when the globus token exists", () => {
        const globus_oauth_token = new GlobusTokenModel(get_globus_token_model_user_id, get_globus_token_model_collection_id).get_oauth_token();
    ...
    it("should return an empty read only DataFed OAuth token with all fields undefined object when the globus token does not exist", () => {
        const globus_oauth_token = new GlobusTokenModel("u/user_dne", "globus_coll/coll_dne").get_oauth_token();
