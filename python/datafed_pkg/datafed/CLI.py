    ...
    while True:
        ...
        try:
            ...
            if _output_mode == _OM_TEXT:
                ...
                click.echo(
                    '{{"msg_type":"ClientError","message":"{}"}}'.format(
                        e.format_message()
                    )
                )
            if _first:
...
def _genDocCmd(cmd, ctx, level, parname=None, recurse=True):
    ...
    if is_group:
        ...
        for c in cmd.list_commands(ctx):
            ...
            if not subcmd.hidden:
                doc += "{0: <15}  {1}\n".format(
                    subcmd.name, subcmd.get_short_help_str(limit=60)
                )
        doc += "===============  ============================================================\n\n"
    ...
    if coll_id is None:
        ...
        if coll.owner != _cur_ctx:
            ...
            if _output_mode == _OM_TEXT and _cur_ctx != _uid:
                click.echo(
                    "CLI now tracking as {}. Use 'cd //' to return to {}.".format(
                        _cur_ctx, _uid
                    )
                )
...
@click.option(
    ...
    type=str,
    required=False,
    help="Override raw data file extension if provided (default is auto detect).",
)
@click.option(
    "-E",
    "--external",
    is_flag=True,
    required=False,
    help="Raw data file is external to DataFed (unmanaged)",
)
@click.option(
    "-m",
    "--metadata",
...
@click.option(
    ...
    is_flag=True,
    required=False,
    help="Fail on metadata validation errors",
)
@click.option(
    "-p",
    "--parent",
...
@click.option(
    ...
    type=str,
    required=False,
    help="Repository ID (managed data only). Uses default allocation if not specified.",
)
@click.option(
    "-D",
    "--deps",
...
def _dataCreate(
    ...
    if raw_data_file and not (_interactive and external):
        raise Exception(
            "Cannot specify --raw-data-file option in non-interactive modes."
        )
...
@click.option(
    ...
    "--wait",
    is_flag=True,
    help="Block reply or further commands until transfer is complete",
)
@click.option(
    "-x",
    "--extension",
...
@click.option(
    ...
    type=str,
    required=False,
    help="Optional target collection (default is root).",
)
...
def _list(ctx, item_id, offset, count, context):
    ...
    if _id[:2] == "p/":
        if _capi.projectGetRole(_id) == 0:
            ...
            reply = _capi.collectionItemsList(
                "c/p_" + _id[2:] + "_root", offset=offset, count=count, context=context
            )
            if _output_mode == _OM_TEXT:
...
@click.option(
    ...
    type=str,
    required=False,
    help="Publish the collection to the provided topic.",
)
@_global_context_options
@_global_output_options
...
@click.option(
    ...
    type=str,
    required=False,
    help="Publish the collection under the provided topic.",
)
@_global_context_options
@_global_output_options
def _collUpdate(coll_id, title, alias, description, tags, topic, context):
    ...
    reply = _capi.collectionUpdate(
        _resolve_coll_id(coll_id, context=context),
        title=title,
        alias=alias,
        description=description,
        tags=tags,
...
@click.option(
    ...
    multiple=True,
    type=str,
    help="Collection to search (multiple allowed)",
)
...
def _projectList(owned, admin, member, offset, count):
    ...
    reply = _capi.projectList(
        owned=owned, admin=admin, member=member, offset=offset, count=count
    )
    _generic_reply_handler(reply, _print_listing)
    ...
    for dr in message.data:
        ...
        if dr.external:
            click.echo(
                ...
                + "{:<15}{:<50}".format(
                    "Source: ", dr.source if dr.source else "(none)"
                )
            )
        else:
            click.echo(
            ...
            if dr.ext_auto:
                ...
                click.echo(
                    "{:<15}{:<50}".format(
                        "Extension: ", dr.ext if dr.ext else "(not set)"
                    )
                )
...
def _print_coll(message):
    for coll in message.coll:
        ...
        click.echo(
            "{:<15}{:<50}".format(
                "Topic: ", coll.topic if coll.topic else "(not published)"
            )
            + "\n"
...
def _print_deps(dr):
    ...
    for i in dr.deps:
        if (
            i.dir == 0
        ):  # incoming -- DR is old, precursor, or container -- DEP is relative of DR
            click.echo(
                "  {:12} ({:<15} {:20} {:12} ({:<15}".format(
                    i.id, i.alias + ")", types[i.type], dr.id, dr.alias + ")"
                )
            )
        elif (
            i.dir == 1
        ):  # outgoing -- DR is new, derivation, or component -- DR is relative of DEP
            click.echo(
                "  {:12} ({:<15} {:20} {:12} ({:<15}".format(
    ...
    for t in message.task:
        ...
        click.echo(
            "{:2}. {:14}  {:13}  {:9}  {}  {}".format(
                ...
                task_type,
                task_status,
                _capi.timestampToStr(t.ct),
                _capi.timestampToStr(t.ut),
            )
        )
        df_idx += 1
...
def _print_task(message):
    if message.HasField("task"):
        ...
        click.echo(
            "{:<20} {:<50}".format("Task ID: ", message.task.id)
            + "\n"
...
def _print_task_array(message):
    ...
    for t in message.task:
        ...
        click.echo(
            "{:<20} {:<50}".format("Started: ", _capi.timestampToStr(t.ct))
            + "\n"
...
def _print_user(message):
    for usr in message.user:
        if _verbosity >= 0:
            click.echo(
                "{:<10} {:<50}".format("User ID: ", usr.uid)
                + "\n"
                + "{:<10} {:<50}".format(
                    "Name: ", usr.name_last + ", " + usr.name_first
                )
                + "\n"
                    ...
                    if first:
                        ...
                        click.echo(
                            "{:<14} {}, {} total, {} used".format(
                                "Allocations:",
                                alloc.repo,
                                _capi.sizeToStr(alloc.data_limit),
                                _capi.sizeToStr(alloc.data_size),
                            )
                        )
                    else:
                        click.echo(
...
def _print_query(message):
    click.echo(
        ...
        + "{:<20} {}\nSearch Terms:\n".format(
            "Updated: ", _capi.timestampToStr(message.ut)
        )
    )

    click.echo(
        "  {:<18} {}".format(
            "Mode: ", "Data" if message.query.mode == 0 else "Collections"
        )
    )
...
def _wrap_text(text, prefix, indent, compact=False):
    ...
    wrapper = textwrap.TextWrapper(
        initial_indent=prefix, subsequent_indent=" " * indent, width=w
    )
...
def _bar_adaptive_human_readable(current, total, width=80):
    ...
    for field in selected:
        if field == "percent":
            ...
            output += (
                "%s / %s" % (_capi.sizeToStr(current), _capi.sizeToStr(total))
    ...
    try:
        ...
        if man_auth:
            ...
            if _output_mode == _OM_RETN:
                raise Exception(
                    "The --manual-auth option may not be used when running in API mode."
                )
            elif not _interactive:
                raise Exception(
                    "The --manual-auth option may not be used when running non-interactively."
                )
        ...
        if _interactive and _capi._mapi.new_client_avail:
            click.echo(
                ">>>> A newer DataFed client ({}) is available <<<<".format(
                    _capi._mapi.new_client_avail
                )
            )
        ...
        if man_auth or _capi.getAuthUser() is None:
            if not man_auth:
                if not _capi._mapi.keysLoaded():
                    if _output_mode == _OM_RETN:
                        raise Exception(
                            "Not authenticated: no local credentials loaded."
                        )
