...
EOF
  ...
  if [[ -n "${local_DATAFED_ARANGO_KEY_PATH}" || "${ARANGO_USE_SSL}" == "TRUE" ]]; then
    echo "DATAFED_ARANGO_KEY_PATH=${local_DATAFED_ARANGO_KEY_PATH}" >>$ENV_FILE_PATH
  fi
  if [[ -n "${local_DATAFED_ARANGO_CERT_PATH}" || "${ARANGO_USE_SSL}" == "TRUE" ]]; then
    echo "DATAFED_ARANGO_CERT_PATH=${local_DATAFED_ARANGO_CERT_PATH}" >>$ENV_FILE_PATH
  fi
  if [[ -n "${local_DATAFED_ARANGO_PEM_PATH}" || "${ARANGO_USE_SSL}" == "TRUE" ]]; then
