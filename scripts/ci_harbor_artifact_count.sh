...
then
  echo "The DATAFED Harbor Registry has not been defined."
  exit 1
fi

if [ -z "$local_DATAFED_HARBOR_REPOSITORY" ]
then
  echo "The DATAFED Harbor Repository has not been defined."
  exit 1
fi

if [ -z "$local_DATAFED_HARBOR_USERNAME" ]
then
  echo "The DATAFED Harbor Username has not been defined."
  exit 1
fi

if [ -z "$local_DATAFED_HARBOR_PASSWORD" ]
then
  echo "The DATAFED Harbor Password has not been defined."
  exit 1
fi
...
if [ "$error_code" != "0" ]
then
	echo "Something went wrong when communicating with harbor registry"
	echo "$URL with tag $local_DATAFED_HARBOR_IMAGE_TAG"
	if [ "$error_code" == "6" ]
	then
		echo "cURL error code 6, could not resolve host, make sure the domain is correct $local_DATAFED_HARBOR_URL, and that the network is open, and that the site is up."
	elif [ "$error_code" == "1" ]
	then
		echo "cURL error code 1, generic error code detected, make sure the password username combination is correct."
	fi
fi
