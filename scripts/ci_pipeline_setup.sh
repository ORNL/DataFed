...
if [ -z "$GITLAB_DATAFEDCI_REPO_API_TOKEN" ]; then
...
wait_for_running_infrastructure_pipelines_to_finish() {
  ...
  local count=0
  while [ ! -z "$all_other_pipelines" ]; do
...
find_orc_instance_by_id() {
  ...
  if [ "$instance_id" == "$COMPUTE_INSTANCE_ID" ]; then
    found_vm_id="TRUE"
...
VM_IS_ACTIVE="TRUE"
if [[ "$INSTANCE_STATUS_SANITIZED" != "ACTIVE" ]]; then
  VM_IS_ACTIVE="FALSE"
fi

# Only trigger the pipeline here if it was not triggered before
if [[ "$VM_IS_ACTIVE" == "FALSE" && -z "$pipeline_id" ]]; then
