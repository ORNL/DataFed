  ...
  namespace {
    ...
    void receiveCorrelationID(IMessage & msg, void * incoming_zmq_socket) {
      ...
      if ( zmq_msg_size( &zmq_msg )) {
        std::string correlation_id = std::string((char*) zmq_msg_data( &zmq_msg ), zmq_msg_size( &zmq_msg ));
      ...
      if( !zmq_msg_more(&zmq_msg) ) {
        EXCEPT( 1, "Should be receiving messages after correlation id. The key should follow but is not." );
      }
      zmq_msg_close( &zmq_msg );
    }
    ...
    void sendCorrelationID(IMessage & msg, void * outgoing_zmq_socket) {
      ...
      if ( msg.exists(MessageAttribute::CORRELATION_ID) ){
        const std::string correlation_id = std::get<std::string>(msg.get(MessageAttribute::CORRELATION_ID));
        zmq_msg_init_size( &zmq_msg, correlation_id.size() );
        memcpy( zmq_msg_data( &zmq_msg ), correlation_id.c_str(), correlation_id.size() );
      } else {
        EXCEPT( 1, "zmq_msg_send (correlation id) failed. Message missing correlation id, something is really wrong." );
      }
