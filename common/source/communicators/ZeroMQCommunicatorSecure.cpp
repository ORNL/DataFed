...
void ZeroMQCommunicatorSecure::zmqCurveSetup(const ICredentials& credentials) {
  ...
  if (m_socket->getSocketClassType() == SocketClassType::SERVER) {
    ...
    if ((rc = zmq_setsockopt(m_zmq_socket, ZMQ_CURVE_SERVER, &curve_server,
      ...
      EXCEPT_PARAM(1,
                   "Set ZMQ_CURVE_SERVER failed when constructing ZeroMQ "
                   "Server. ZMQ msg: "
    ...
    if ((rc = zmq_setsockopt(m_zmq_socket, ZMQ_CURVE_SECRETKEY, private_key,
                             32)) == -1) {
      EXCEPT_PARAM(1,
                   "Set ZMQ_CURVE_SECRETKEY failed when constructing ZeroMQ "
                   "Server. ZMQ msg: "
                       << zmq_strerror(zmq_errno()));
    }
    ...
    if (!zmq_z85_decode(server_key, local_serv_key.c_str())) {
      ...
      EXCEPT_PARAM(1,
                   "Decode server public key failed when constructing ZeroMQ "
                   "Client. ZMQ msg: "
                       << zmq_strerror(zmq_errno()));
    }
    ...
    if ((rc = zmq_setsockopt(m_zmq_socket, ZMQ_CURVE_SECRETKEY, private_key,
                             32)) == -1) {
      EXCEPT_PARAM(1,
                   "Set ZMQ_CURVE_PUBLICKEY failed when constructing ZeroMQ "
                   "Client. ZMQ msg: "
                       << zmq_strerror(zmq_errno()));
    }
...
ZeroMQCommunicatorSecure::ZeroMQCommunicatorSecure(
  ...
  if (not zmq_has("curve")) {
    EXCEPT(1,
           "ZeroMQ was not built with curve support cannot create secure "
           "connections.");
  }
