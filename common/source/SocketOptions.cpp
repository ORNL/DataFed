...
AddressSplitter::AddressSplitter(const std::string &address) {
  ...
  if (copy.size() < 1) {
    EXCEPT_PARAM(1, "Error unable to deconstruct host, it does not appear to "
                    "be provided in the address.");
  }
    ...
    if (port.size() > 0) {
      ...
      if (std::all_of(port.begin(), port.end(), ::isdigit)) {
        ...
        if (local_value < max_16_bit_val) {
          ...
          EXCEPT_PARAM(1, "Port number exceeds allowed numeric limit of 16 bit "
                          "unsigned int: "
                              << local_value);
        }
      } else {
        EXCEPT_PARAM(
            1,
            "Deconstructing address failed, port contains non-numeric types: "
                << port);
      }
    }
