...
ProxyBasicZMQ::ProxyBasicZMQ(
  ...
  if (m_client_zmq_type != ZMQ_DEALER) {
    EXCEPT_PARAM(1,
                 "ProxyBasicZMQ frontend currently only supports DEALER type, "
                 "the type provided is: "
  ...
  if (m_server_zmq_type != ZMQ_ROUTER) {
    EXCEPT_PARAM(1,
                 "ProxyBasicZMQ backend currently only supports ROUTER type, "
                 "the type provided is: "
  ...
  if (socket_options.at(SocketRole::SERVER).connection_life !=
    ...
    EXCEPT(1,
           "ProxyBasicZMQ server currently only supports persistent "
...
void ProxyBasicZMQ::run() {
  ...
  if (m_run_infinite_loop == false) {
    std::cout << "**************************************" << std::endl;
    std::cout << "Launching control thread for duration "
              << m_run_duration.count() << std::endl;
   ...
   **/
  std::cout << "Binding ROUTER to address: " << m_server_socket->getAddress()
            << std::endl;
  int rc =
   ...
   **/
  std::cout << "Binding DEALER socket to address: "
            << m_client_socket->getAddress() << std::endl;
