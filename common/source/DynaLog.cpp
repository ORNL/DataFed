...
std::list<Logger::StreamEntry>::iterator
Logger::addStream(std::ostream &stream) {
  std::lock_guard<std::mutex> lock(m_streams_mutex); // Lock the list mutex
  auto it = m_streams.insert(m_streams.end(),
                             StreamEntry{stream}); // Insert and get iterator
