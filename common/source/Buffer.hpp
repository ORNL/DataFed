...
public:
   ...
   **/
  template <typename T> friend void copyFromBuffer(T, const Buffer &);
};
...
inline void copyToBuffer(Buffer &buffer, const T data, size_t size) {
  if (size > 0) {
    buffer.reserve(size);
    try {
      if constexpr (std::is_base_of<::google::protobuf::Message,
                                    std::remove_pointer_t<T>>::value) {
        try {
          if (!data->IsInitialized()) {
            EXCEPT(
                1,
                "Cannot copy message to buffer it is missing required fields");
          }
...
template <typename T> inline void copyFromBuffer(T data, const Buffer &buffer) {
  if (buffer.m_size > 0) {
    try {
      if constexpr (std::is_base_of<::google::protobuf::Message,
                                    std::remove_pointer_t<T>>::value) {
