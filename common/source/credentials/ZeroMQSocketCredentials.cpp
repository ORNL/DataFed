...
void ZeroMQSocketCredentials::validate() {
  if (m_public_key) {
    if (m_public_key->size() != 40) {
      EXCEPT_PARAM(1, "ZMQ Curve public key must have exactly 40 chars, the "
                      "provided key has: "
                          << m_public_key->size());
    }
  }
  if (m_private_key) {
    if (m_private_key->size() != 40) {
      EXCEPT_PARAM(1, "ZMQ Curve private key must have exactly 40 chars, the "
                      "provided key has: "
                          << m_private_key->size());
    }
  }
  if (m_server_key) {
    if (m_server_key->size() != 40) {
      EXCEPT_PARAM(1, "ZMQ Curve server key must have exactly 40 chars, the "
                      "provided key has: "
                          << m_server_key->size());
    }
  }
...
std::string
ZeroMQSocketCredentials::get(const CredentialType credential_type) const {
  if (credential_type == CredentialType::PUBLIC_KEY) {
    ...
    EXCEPT(1, "Cannot grab private key from ZMQ socket credentials it is not "
              "defined");
