...
namespace SDMS {
  ...
  std::unique_ptr<IServer> ServerFactory::create(
    ...
    if( server_type == ServerType::PROXY_CUSTOM ) {
      return std::unique_ptr<IServer>(new Proxy(socket_options, socket_credentials, std::move(incoming_operators), m_log_context));
      ...
      return std::unique_ptr<IServer>(new Proxy(socket_options, socket_credentials, m_log_context));
    }
