 ...
 * Getters
 **/
std::variant<std::string, MessageState> GoogleProtoMessage::get(
    MessageAttribute attribute_type) const {
  if (attribute_type == MessageAttribute::STATE) {
    return m_state;
...
std::variant<uint8_t, uint16_t, uint32_t> GoogleProtoMessage::get(
