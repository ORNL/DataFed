...
class GoogleProtoMessage : public IMessage {
 ...
 private:
  ...
  virtual std::variant<std::string, MessageState> get(
      MessageAttribute) const final;
  virtual std::variant<uint8_t, uint16_t, uint32_t> get(
      const std::string &attribute_name) const final;
  ...
  virtual std::variant<::google::protobuf::Message *, std::string> getPayload()
      final;
};
