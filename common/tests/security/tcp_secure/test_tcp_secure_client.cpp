...
int main(int a_argc, char ** a_argv ) {
  ...
  { // Receive a NACK response 
    ...
    if( response_client.time_out == true ) {
      ...
      std::cout << client->id() << " Message received from server..." << std::endl;
    }
    ...
    std::cout << client->id() << " Validating message content received from server..." << std::endl;

    if(response_payload->err_code() != ErrorCode::ID_SERVICE_ERROR) {
      std::cout << client->id() << " FAILED" << std::endl;
      EXCEPT_PARAM(1, "TCP Secure test failed unexpected ErrorCode returned by NACK reply from server, client failing.");
    }
    if( response_payload->err_msg().compare(error_msg) != 0) {
      std::cout << client->id() << " FAILED" << std::endl;
      EXCEPT_PARAM(1, "TCP Secure test failed unexpected error message returned from server provided: " << response_payload->err_msg() << " Expected: " << error_msg );
    }
  } 
