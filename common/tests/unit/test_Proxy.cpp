...
BOOST_AUTO_TEST_CASE( testing_ProxyChain ) {
  ...
  std::unique_ptr<std::thread> proxy_thread = std::unique_ptr<std::thread>(new std::thread(
        ...
        // Pass the arguments to the Thread
        }, proxy_client_id, proxy_server_id, test_duration
  )); // thread for first proxy server
  ...
  std::unique_ptr<std::thread> proxy_thread2 = std::unique_ptr<std::thread>(new std::thread(
        [](
        ...
        // Pass the arguments to the Thread
        }, proxy_client_id2, proxy_server_id2, test_duration
  )); // thread for second proxy server
