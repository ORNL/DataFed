...
public:
    ...
    // streams.
    struct StreamEntry {
        std::reference_wrapper<std::ostream> stream;  // Reference wrapper for the stream
        std::unique_ptr<std::mutex> mutex;            // Mutex for thread-safe operations

        StreamEntry(std::ostream &out_stream)
            : stream(out_stream), mutex(std::make_unique<std::mutex>()) {}
    };
...
private:
  ...
  // List was chosen because we will need to be adding and removing streams
  // as classes are created and destroyed this does not work well in a
...
public:
  ...
  // Add a new stream and return an iterator to it for easy removal 
  // in the future.
  ...
  std::list<StreamEntry>::iterator  addStream(std::ostream &stream);
  
  // Remove a stream by iterator, this is fast for a linked list.
  void removeStream(std::list<StreamEntry>::iterator it);
