...
#define EXCEPT(err_code, msg)                                                  \
  throw TraceException(__FUNCTION__, __LINE__, err_code, msg)

#define EXCEPT_PARAM(err_code, msg)                                            \
  do {                                                                         \
    std::stringstream _trace_excep_sstr;                                       \
    _trace_excep_sstr << msg;                                                  \
    throw TraceException(__FUNCTION__, __LINE__, err_code,                     \
                         _trace_excep_sstr.str());                             \
  } while (0)
...
public:
  TraceException(const char *a_file, unsigned long a_line,
                 unsigned long a_error_code, const std::string &a_context)
      : m_file(a_file), m_line(a_line), m_error_code(a_error_code),
        m_context(a_context) {}
  ...
  std::string toString(bool debug = false) const {
    if (debug) {
      ...
      sstr << "(source: " << m_file << ":" << m_line << " code:" << m_error_code
           << ")" << std::endl;
...
//#define RAPIDJSON_ASSERT(x) if (!(x)) throw TraceException( __FUNCTION__,
//__LINE__, 0, #x )
