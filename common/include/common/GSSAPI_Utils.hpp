...
public:
    ...
    operator gss_buffer_t()
    {
        return &m_gss_buf;
    }

    operator const gss_buffer_desc *() const
    {
        return &m_gss_buf;
    }
...
private:
    void set( const std::string & a_src )
    {
        ...
        if ( !m_gss_buf.value )
            throw std::runtime_error("gssString malloc failed");
    ...
    void set_name( gss_name_t a_src )
    {
        ...
        if ( gss_display_name( &min_stat, a_src, &m_gss_buf, &name_type ) != GSS_S_COMPLETE )
