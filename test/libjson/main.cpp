...
#define timerDef()                                            \
  LARGE_INTEGER _T0 = {0, 0}, _T1 = {0, 0}, _T2 = {0, 0}, _F; \
  (void)_T0;                                                  \
  (void)_T1;                                                  \
  (void)_T2;                                                  \
...
#define timerNow() \
  (QueryPerformanceCounter(&_T2) ? (_T2.QuadPart / (double)_F.QuadPart) : 0)

#else

// Define local vars need by timer functions
#define timerDef()                                          \
  struct timespec _T0 = {0, 0}, _T1 = {0, 0}, _T2 = {0, 0}; \
  (void)_T0;                                                \
  (void)_T1;                                                \
  (void)_T2;
...
// Calc elapsed time
#define timerElapsed() \
  ((_T1.tv_sec - _T0.tv_sec) + ((_T1.tv_nsec - _T0.tv_nsec) * 1.0e-9))

// get current absolute time in sec
#define timerNow()                             \
  (clock_gettime(CLOCK_REALTIME, &_T2) == 0    \
       ? (_T2.tv_sec + (_T2.tv_nsec * 1.0e-9)) \
       : 0)
...
void perfTest() {
  ...
  try {
    ...
    cout << "Completed " << ntest << " parses in " << elapsed << " sec, "
         << 1000.0 * elapsed / ntest << " msec per parse\n";
    ...
    cout << "Completed " << ntest << " translations in " << elapsed << " sec, "
         << 1000.0 * elapsed / ntest << " msec per translation\n";
